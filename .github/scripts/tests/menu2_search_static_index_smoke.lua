...
local lines, pages, identities = 0, {}, {}
for line in blob:gmatch("[^\n]+") do
    lines = lines + 1
    local fields = {}
    for field in (line .. "\t"):gmatch("([^\t]*)\t") do fields[#fields + 1] = field end
    assert(#fields == 9, string.format(
        "row %d has %d fields, expected 9: %s", lines, #fields, line:sub(1, 90)))
    local pageKey, label, kind, _, _, _, labelNorm, searchIdentity, haystack = unpack(fields)
    ...
    assert(searchIdentity ~= "", "row " .. lines .. " has no semantic search identity")
    assert(not searchIdentity:find(".", 1, true),
        "row " .. lines .. " exposes a dotted derived route that Graphify can misclassify as a setting")
    assert(not identities[searchIdentity], string.format(
        "rows %d and %d share search identity %q", identities[searchIdentity] or -1, lines, searchIdentity))
    identities[searchIdentity] = lines
...
for _, field in ipairs({ "key", "label", "kind", "labelNorm", "haystack", "hintNorm",
    "titleNorm", "groupNorm", "tokenLimit", "searchIdentity" }) do
    assert(sample[field] ~= nil, "decoded record is missing field " .. field)
end

-- The generator must be reproducible from a clean checkout. `tools/` is ignored
-- by default, so the tracked wrapper must point at the tracked projector.
local generator = Read("tools/generate_search_static_index.ps1")
assert(generator:find('$projector = ".github/scripts/search_static_index_project.lua"', 1, true),
    "search index generator still depends on an ignored tools projector")
assert(blob and Read(SEARCH_DIR .. "MSUF_Menu2_Search_StaticIndex_Data.lua"):find(
    "Generated by .github/scripts/search_static_index_project.lua", 1, true),
    "generated search index does not name its tracked projector")
...
assert(indexQuery:find("covered[rec.searchIdentity] = true", 1, true),
    "live widget records no longer suppress their exact static route")
assert(not indexQuery:find('rec.key .. "" .. rec.labelNorm', 1, true),
    "page+label deduplication returned; equal labels would hide distinct controls")
