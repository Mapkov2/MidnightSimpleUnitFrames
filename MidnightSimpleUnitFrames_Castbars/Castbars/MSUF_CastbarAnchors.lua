...
local function GetUnitFrames()
    UnitFrames = _G.MSUF_UnitFrames or UnitFrames or _G.UnitFrames
    return UnitFrames
end

local function MSUF_ShouldUseCastbarUnit(unit, g)
    local fn = _G.MSUF_ShouldUseMSUFCastbar
    if type(fn) == "function" then
        return fn(unit, g) == true
    end
    if unit == "player" then return not (g and g.enablePlayerCastbar == false) end
    if unit == "target" then return not (g and g.enableTargetCastbar == false) end
    if unit == "focus" then return not (g and g.enableFocusCastbar == false) end
    if unit == "boss" then return not (g and g.enableBossCastbar == false) end
    return true
end
...
function MSUF_AttachBlizzardTargetFrame()
    ...
    local msufTarget = frames and frames["target"]
...
function MSUF_ReanchorTargetCastBar()
    ...
    if not MSUF_ShouldUseCastbarUnit("target", g) then
...
function MSUF_ReanchorFocusCastBar()
    ...
    if not MSUF_ShouldUseCastbarUnit("focus", g) then
        frame:SetScript("OnUpdate", nil)
        if frame.timeText and MSUF_IsCastTimeEnabled(frame) then
            MSUF_SetTextIfChanged(frame.timeText, "")
        end
    ...
    local frames = GetUnitFrames()
    local msufFocus = frames and frames["focus"]
...
local castbarWidthSourcePendingAfterCombat = false
...
local MSUF_ApplyCastbarEffectiveSizeUnit

local function MSUF_CastbarWidthSourceCombatLocked()
...
local function MSUF_GetCastbarWidthSourceSignature(g, unit)
    unit = MSUF_NormalizeCastbarUnit(unit)
    if not MSUF_ShouldUseCastbarUnit(unit, g) then return nil end
...
local function MSUF_CastbarWidthSourceChanged()
    if MSUF_CastbarWidthSourceCombatLocked() then
        castbarWidthSourcePendingAfterCombat = true
...
local function MSUF_RefreshCastbarWidthSourceUnit(unit, g)
    if MSUF_CastbarWidthSourceCombatLocked() then
        castbarWidthSourcePendingAfterCombat = true
        return
    end
    unit = MSUF_NormalizeCastbarUnit(unit)
    if not MSUF_ShouldUseCastbarUnit(unit, g) then return end
    if not MSUF_UnitHasActiveCastbarWidthSource(g, unit) then return end

    if MSUF_ApplyCastbarEffectiveSizeUnit then
        MSUF_ApplyCastbarEffectiveSizeUnit(unit, g)
    end
end
...
local function MSUF_FlushCastbarWidthSourceSync()
    ...
    for _, unit in ipairs(CASTBAR_WIDTH_UNITS) do
        if MSUF_CastbarWidthSourceNeedsReanchor(g, unit) then
            MSUF_RefreshCastbarWidthSourceUnit(unit, g)
        end
    end
end
...
do
    ...
    boot:SetScript("OnEvent", function(_, event)
        if event == "PLAYER_REGEN_ENABLED" then
            if not castbarWidthSourcePendingAfterCombat and not castbarWidthSourceQueued then
                return
            end
        elseif MSUF_CastbarWidthSourceCombatLocked() then
            castbarWidthSourcePendingAfterCombat = true
            return
        end

        castbarWidthSourcePendingAfterCombat = false
        EnsureDB()
        local g = MSUF_DB and MSUF_DB.general or {}
...
MSUF_ApplyCastbarEffectiveSizeUnit = function(unit, g)
    if MSUF_CastbarWidthSourceCombatLocked() then
        castbarWidthSourcePendingAfterCombat = true
        return false
    end
    ...
    if not MSUF_ShouldUseCastbarUnit(unit, g) then return false end

    local function SetOuterSize(frame, w, h)
        if not frame or not w or w <= 0 then return false end
        if MSUF_SetWidthIfChanged then MSUF_SetWidthIfChanged(frame, w) else frame:SetWidth(w) end
        if h and h > 0 then
            if MSUF_SetHeightIfChanged then MSUF_SetHeightIfChanged(frame, h) else frame:SetHeight(h) end
        end
        return true
    end
    ...
    if unit == "player" then
        ...
        if frame then MSUF_ApplyPlayerCastbarSizeAndLayout(frame, g, w, h) end
        if preview then MSUF_ApplyPlayerCastbarSizeAndLayout(preview, g, w, h) end
        return true
    end
    ...
    if unit == "target" or unit == "focus" then
        ...
        if frame and SetOuterSize(frame, w, h) and frame.statusBar then
            ...
            local statusW = math.max(1, (w or fallbackW or 240) - barH - 1)
            MSUF_SetWidthIfChanged(frame.statusBar, statusW)
        end
...
function MSUF_ReanchorPlayerCastBar()
    ...
    if not (_G.MSUF_ShouldUseBlizzardCastbar and _G.MSUF_ShouldUseBlizzardCastbar("player", g)) then
...
_G.MSUF_ApplyCastbarEffectiveSizeUnit     = MSUF_ApplyCastbarEffectiveSizeUnit
