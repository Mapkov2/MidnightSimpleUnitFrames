...
local function BossPreviewContextActive()
...
local function BossLayoutDelta(index)
    local fn = _G.MSUF_GetBossLayoutDelta
    if type(fn) == "function" then
        local dx, dy = fn(index)
        return tonumber(dx) or 0, tonumber(dy) or 0
    end
    local conf = _G.MSUF_DB and _G.MSUF_DB.boss or {}
    local step = (tonumber(index) or 1) - 1
    local spacing = tonumber(conf.spacing) or -42
    local mode = conf.bossLayoutMode or "VERTICAL_DOWN"
    if mode == "VERTICAL_UP" then
        return 0, step * -spacing
    elseif mode == "HORIZONTAL_RIGHT" then
        return step * ((tonumber(conf.width) or 180) + 8), 0
...
local function MSUF_CreateBossCastbarPreview(index)
    ...
    castText:SetText(Tr("Celestial Ruin"))
    f.castText = castText
...
local function MSUF_PositionBossCastbarPreview(f, index)
    ...
    if g.bossCastbarDetached == true then
        -- Detached: keep the same boss layout delta as the unitframe stack.
        local dx, dy = BossLayoutDelta(index)
        f:ClearAllPoints()
        f:SetPoint("CENTER", UIParent, "CENTER", ox + dx, oy + dy)
        return
    end
    ...
    if uf and uf.IsShown and uf:IsShown() then
        ...
        local dx, dy = BossLayoutDelta(index)
        f:ClearAllPoints()
        f:SetPoint("TOPRIGHT", UIParent, "TOPRIGHT", -320 + dx, -200 + dy)
    end
end
...
function _G.MSUF_UpdateBossCastbarPreview()
    ...
    -- Hard gate: never show boss preview when Boss Frames are disabled, or no preview context is active.
    if ((_G.MSUF_DB and _G.MSUF_DB.boss and _G.MSUF_DB.boss.enabled == false) or not BossPreviewContextActive()) then
    ...
    -- Respect boss castbar backend.
    local shouldUse = _G.MSUF_ShouldUseMSUFCastbar
    if type(shouldUse) == "function" and not shouldUse("boss", g) then
        MSUF_HideAllBossCastbarPreviews()
        return
    elseif type(shouldUse) ~= "function" and g.enableBossCastbar == false then
        MSUF_HideAllBossCastbarPreviews()
        return
    end
