...
synchronized through Retail main `1815151a` (the released 6.15-beta2 snapshot
`348b3643` plus the Text on detached bar fix). The public 6.5 Alpha line combines the complete responsive
...
It exposes `MSUF.Client`, `MSUF.Vanilla`, `MSUF.Mists`, `MSUF.TBC`, and `MSUF.Classic`, and only
...
`Blizzard_AuraContainer` runtime under `Game/Mainline/Auras`. The Vanilla,
Mists, and TBC loaders select `Game/Classic/Auras/MSUF_Auras3_UnitFrames.lua`; it owns
...
Classic-safe plan. Vanilla/Mists/TBC scan only with tokens their AuraUtil accepts;
...
`tools/test-classic-prototype.ps1` validates all twelve TOCs, recursive XML load
