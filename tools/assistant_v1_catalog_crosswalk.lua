...
local function ResolveRepositoryRoot()
    for _, root in ipairs({ ".", "..", "../.." }) do
        if Exists(Join(root, "MidnightSimpleUnitFrames/MidnightSimpleUnitFrames_Mainline.toc"))
            ...
            and Exists(Join(root, "MidnightSimpleUnitFrames_Assistant/MidnightSimpleUnitFrames_Assistant_Mainline.toc"))
            and Exists(Join(root, "tools/classic-client-matrix.tsv"))
        then
            return root
        end
    end
    error("repository root not found (core, Assistant companion and the client matrix are required)")
end
...
-- The tracked Graphify setting inventory was retired in the Classic repository on
-- 2026-09-20: its generator (tools/generate_assistant_graphify_inventory.lua) was
-- never ported here, no Classic gate step or smoke read the snapshot, and about 145
-- of its 2179 records had drifted past the end of the file they named. Everything
-- below is the Graphify-backed release gate, which runs in the Retail repository
-- only. The Menu2 search index generator stops above this point (HARNESS_CUT in
-- .github/scripts/search_static_index_project.lua), so it is unaffected.
error("the tracked Graphify setting inventory was retired in the Classic repository; "
    .. "the Graphify-backed crosswalk gates run in the Retail repository only")
local trackedGraphInventory = nil
...
local function PrepareUnitGroupSettingPage(setting, page)
    ...
    -- Aura ordering settings are exposed by controls labelled "Sort By" and
    -- "Order", while their Registry labels use "Sort Method"/"Sort
    -- Direction". Materialize the canonical Ordering workspace before the
    -- exact dynamic-key lookup; the lookup itself still fails closed on the
    -- declarative catalog association below.
    if tostring(setting and setting.key or ""):match("%.sortMethod$")
        or tostring(setting and setting.key or ""):match("%.sortReverse$")
    then
        query = query .. " ordering sort by sort order"
    end
...
-- Pin the complete finite Aura ordering contract. UnitFrame controls belong
-- to their owning Unit page; Party has its own Group scope; Raid and Mythic
-- Raid are reviewed aliases of the same Raid workspace and therefore must
-- resolve to the same live control for each lane/field pair.
local function AssertAuraOrderingRoute(settingKey, expectedPage)
    local setting = assert(Registry:GetSetting(settingKey),
        "Aura ordering Registry setting is missing: " .. tostring(settingKey))
    local page = A.ResolveMenuPageForSetting(setting)
    assert(page == expectedPage,
        tostring(settingKey) .. " must resolve to owner page " .. tostring(expectedPage)
            .. " (got " .. tostring(page) .. ")")
    local built, buildError = PrepareUnitGroupSettingPage(setting, page)
    assert(built, tostring(settingKey) .. " owner page failed to build: " .. tostring(buildError))
    local record, _, source = Catalog.FindBySettingKey(settingKey, page, setting)
    assert(record and record.pageKey == expectedPage
            and record.assistantDisposition == "dynamic"
            and source == "reviewed_dynamic_key",
        tostring(settingKey) .. " must resolve through its reviewed dynamic owner-page key"
            .. " (page=" .. tostring(record and record.pageKey)
            .. ", source=" .. tostring(source) .. ")")
    return record
end

local auraOrderingRouteCount = 0
for _, unit in ipairs({ "player", "target", "focus", "boss" }) do
    for _, lane in ipairs({ "buff", "debuff" }) do
        for _, suffix in ipairs({ "sortMethod", "sortReverse" }) do
            AssertAuraOrderingRoute("auras3." .. unit .. "." .. lane .. "." .. suffix, "uf_" .. unit)
            auraOrderingRouteCount = auraOrderingRouteCount + 1
        end
    end
end
for _, lane in ipairs({ "buff", "debuff" }) do
    for _, suffix in ipairs({ "sortMethod", "sortReverse" }) do
        AssertAuraOrderingRoute("gf_party.auras." .. lane .. "." .. suffix, "gf_auras")
        auraOrderingRouteCount = auraOrderingRouteCount + 1
        local raidRecord = AssertAuraOrderingRoute("gf_raid.auras." .. lane .. "." .. suffix, "gf_auras")
        local mythicRecord = AssertAuraOrderingRoute("gf_mythicraid.auras." .. lane .. "." .. suffix, "gf_auras")
        assert(raidRecord.controlId == mythicRecord.controlId,
            "Raid/Mythic Aura ordering aliases must resolve to one workspace control for "
                .. lane .. "." .. suffix)
        auraOrderingRouteCount = auraOrderingRouteCount + 2
    end
end
assert(auraOrderingRouteCount == 28, "Aura ordering route crosswalk must cover exactly 28 settings")
for i = 1, #settings do
    local setting = settings[i]
...
local globalReverse = {
    ...
    failures = {}, failureByPage = {}, totalByPage = {}, standaloneExplicitKeys = {},
}
        ...
        if GLOBAL_GAP_PAGES[page] then
            ...
            then
                ...
                if setting.generated == true then
                    ...
                    globalReverse.standaloneExplicitKeys[#globalReverse.standaloneExplicitKeys + 1] = setting.key
                end
...
if verbose then
    ...
    table.sort(globalReverse.standaloneExplicitKeys)
    for i = 1, #globalReverse.standaloneExplicitKeys do
        print("CATALOG_GLOBAL_STANDALONE_EXPLICIT\t" .. globalReverse.standaloneExplicitKeys[i])
    end
...
assert(graphDispositionReview.pass == true, string.format(
    -- Name every offending path: the counts alone give no way to find which
    -- setting needs a reviewed disposition.
    "Graphify raw-candidate disposition gate failed: unclassified=%d stale=%d missingEvidence=%d invalidOwners=%d"
        .. "\n  unclassified: %s\n  stale: %s\n  missingEvidence: %s\n  invalidOwners: %s",
    ...
    table.concat(graphDispositionReview.unclassified, ", "),
    table.concat(graphDispositionReview.stale, ", "),
    table.concat(graphDispositionReview.missingEvidence, ", "),
...
if not unitGroupOnly then
    ...
    -- The observed explicit standalone inventory is 37 canonical controller,
    -- compatibility, or context values without scalar Menu2 widgets. The two
    -- contextual Aura menu-state settings are not members of GLOBAL_GAP_PAGES,
    -- and the 28 Aura ordering associations belong to Unit/Group dynamic-route
    -- parity, so neither changes this Global standalone split.
    ...
    -- 146 generated in total: 145 remain reviewed standalone fallbacks and
    -- one now resolves through a visible catalog route.
    -- 2026-08-25: one explicit standalone left the list when castbar-family
    -- settings started resolving to the Cast Bars page (their real menu
    -- home) instead of the unit page that carries no such widget.
    -- 2026-09-08: the shared inverse-polarity companion
    -- fontScope.shared.shortenNameShowDots is a reviewed explicit standalone
    -- (Menu2 exposes only the No Ellipsis owner): explicit 37 -> 38.
    -- 2026-09-09: the four legacy bars.show<Unit>PowerBar mirrors became
    -- canonical aliases of <unit>.showPowerBar, so they are no longer
    -- generated standalones: 145 -> 141.
    assert(globalReverse.standaloneGenerated == 141 and globalReverse.standaloneExplicit == 38,
        "reviewed Global standalone split changed; classify new generated or explicit no-widget settings"
        .. " (generated=" .. tostring(globalReverse.standaloneGenerated)
        .. " explicit=" .. tostring(globalReverse.standaloneExplicit)
        .. " routed=" .. tostring(globalReverse.generatedRouted) .. ")")
