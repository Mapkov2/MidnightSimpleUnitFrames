...
function Loader.ResolveCompanionRoot(root)
    ...
    for i = 1, #candidates do
        if exists(join(candidates[i], "MidnightSimpleUnitFrames_Assistant_Mainline.toc"))
            and exists(join(candidates[i], "MSUF_AssistantRuntime.xml")) then
            return candidates[i]
        end
    end
    error("MSUF Assistant LoD companion root not found (expected MidnightSimpleUnitFrames_Assistant_Mainline.toc and MSUF_AssistantRuntime.xml)")
end
