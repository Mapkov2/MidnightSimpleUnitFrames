...
local S={win=nil,content=nil,scale={},mirror={host=nil,currentKey="home",currentPanel=nil,homePanel=nil,homeToolsApi=nil,tipText=nil,},}local function MSUF_UpdateHomePanel(panel) if not panel then return end
...
if w.SetEnabled then w:SetEnabled(enabled and true or false); return end
...
return b end
local function UI_CloseButton(parent,a1,rel,a2,x,y,onClick) local b=CreateFrame("Button",nil,parent,"UIPanelCloseButton")if a1 then b:SetPoint(a1,rel or parent,a2 or a1,x or 0,y or 0)end
if onClick then b:SetScript("OnClick",onClick)end
return b end
...
function MSUF_BuildButtonRowTL(parent,x,y,defs,defaultGap) local out,prev={},nil for i,d in ipairs(defs or{})do local gap=d.gap if gap==nil then gap=defaultGap end
...
function MSUF_BuildButtonRow(parent,anchorFrame,a1,a2,x,y,defs,defaultGap) local out,prev={},nil for i,d in ipairs(defs or{})do local gap=d.gap if gap==nil then gap=defaultGap end
if gap==nil then gap=8 end
...
local cx,cy=GetCursorPosition()frame._msufResizing=true frame._msufResizeStartX=cx frame._msufResizeStartY=cy frame._msufResizeStartW=frame:GetWidth()frame._msufResizeStartH=frame:GetHeight()self:SetScript("OnUpdate",function() if not frame._msufResizing then return end
local x,y=GetCursorPosition();local s=(frame.GetEffectiveScale and frame:GetEffectiveScale())or 1 if s==0 then s=1 end
local dw=(x-(frame._msufResizeStartX or x))/s;local dh=((frame._msufResizeStartY or y)-y)/s;local newW=(frame._msufResizeStartW or frame:GetWidth())+dw;local newH=(frame._msufResizeStartH or frame:GetHeight())+dh;local minW=frame._msufMinW or 760 local minH=frame._msufMinH or 520;local maxW=frame._msufMaxW or 2200;local maxH=frame._msufMaxH or 1400 frame:SetSize(clamp(newW,minW,maxW),clamp(newH,minH,maxH)) end
) end
)grip:SetScript("OnMouseUp",Stop);grip:SetScript("OnHide",Stop)do local prev=frame.GetScript and frame:GetScript("OnMouseUp");frame:SetScript("OnMouseUp",function(self,btn) Stop()if prev then pcall(prev,self,btn)end
end
)end
frame._msufResizeGrip=grip frame._msufStopManualResize=Stop end
local function MSUF_EnsureGeneral() MSUF_SafeCall(EnsureDB)if type(MSUF_DB)~="table"then return nil end
MSUF_DB.general=MSUF_DB.general or{}return MSUF_DB.general end
...
local name=frame:GetName()if not name or name==""then return end
if type(UISpecialFrames)=="table"then for _,v in ipairs(UISpecialFrames)do if v==name then return end
end
...
local g=MSUF_EnsureGeneral()if not g then return end
...
)widget:SetScript("OnLeave",function() if GameTooltip then GameTooltip:Hide()end
end
) end
local MSUF_GetNextTip MSUF_PickSessionTip=function() local tip=(_G.MSUF_GetNextTip and _G.MSUF_GetNextTip())if tip then S.mirror.tipText=tip end
end
local MSUF_THEME={bgR=0.03,bgG=0.05,bgB=0.12,bgA=0.95,edgeR=0.10,edgeG=0.20,edgeB=0.45,edgeA=0.90,titleR=0.75,titleG=0.88,titleB=1.00,titleA=1.00,textR=0.86,textG=0.92,textB=1.00,textA=1.00}local function MSUF_Clamp01(x) x=tonumber(x)or 0 if x<0 then return 0 end
if x>1 then return 1 end
return x end
local MSUF_PILL_EDGE_R=MSUF_Clamp01((MSUF_THEME.edgeR or 0.10)*1.25);local MSUF_PILL_EDGE_G=MSUF_Clamp01((MSUF_THEME.edgeG or 0.20)*1.25);local MSUF_PILL_EDGE_B=MSUF_Clamp01((MSUF_THEME.edgeB or 0.45)*1.18);local MSUF_PILL_EDGE_A=MSUF_Clamp01((MSUF_THEME.edgeA or 0.90)+0.05);local MSUF_SUPERELLIPSE_TEX="Interface/AddOns/"..tostring(addonName or"MidnightSimpleUnitFrames").."/Media/superellipse.tga"local function MSUF_EnsureSuperellipseLayers(btn,inset) if not btn or not btn.CreateTexture then return nil,nil end
inset=tonumber(inset)or 2 local function SnapOff(tex) if tex and tex.SetSnapToPixelGrid then tex:SetSnapToPixelGrid(false)if tex.SetTexelSnappingBias then tex:SetTexelSnappingBias(0)end
end
end
local function MakeGroup(key,layer,subLevel) local g=btn[key]if g then return g end
g={}g.L=btn:CreateTexture(nil,layer,nil,subLevel or 0)g.M=btn:CreateTexture(nil,layer,nil,subLevel or 0)g.R=btn:CreateTexture(nil,layer,nil,subLevel or 0)g._msufSEParts={g.L,g.M,g.R}g.L:SetTexture(MSUF_SUPERELLIPSE_TEX)g.M:SetTexture(MSUF_SUPERELLIPSE_TEX)g.R:SetTexture(MSUF_SUPERELLIPSE_TEX)g.L:SetTexCoord(0.0,0.25,0.0,1.0)g.M:SetTexCoord(0.25,0.75,0.0,1.0)g.R:SetTexCoord(0.75,1.0,0.0,1.0)SnapOff(g.L);SnapOff(g.M);SnapOff(g.R)g.SetVertexColor=function(self,r,gg,b,a) for i=1,#self._msufSEParts do local t=self._msufSEParts[i]if t and t.SetVertexColor then t:SetVertexColor(r,gg,b,a)end
end
end
g.Hide=function(self) for i=1,#self._msufSEParts do local t=self._msufSEParts[i];if t and t.Hide then t:Hide()end
end
end
g.Show=function(self) for i=1,#self._msufSEParts do local t=self._msufSEParts[i];if t and t.Show then t:Show()end
end
end
...
local function Layout(g,pad) local w=(btn.GetWidth and btn:GetWidth())or 140;local h=(btn.GetHeight and btn:GetHeight())or 22;local innerW=math.max(1,w-pad*2);local innerH=math.max(1,h-pad*2);local r=math.floor(innerH*0.5+0.5)local capW=math.min(r,math.floor(innerW*0.5))g.L:ClearAllPoints()g.M:ClearAllPoints()g.R:ClearAllPoints()g.L:SetPoint("TOPLEFT",btn,"TOPLEFT",pad,-pad)g.L:SetPoint("BOTTOMLEFT",btn,"BOTTOMLEFT",pad,pad)g.L:SetWidth(capW)g.R:SetPoint("TOPRIGHT",btn,"TOPRIGHT",-pad,-pad)g.R:SetPoint("BOTTOMRIGHT",btn,"BOTTOMRIGHT",-pad,pad)g.R:SetWidth(capW)g.M:SetPoint("TOPLEFT",g.L,"TOPRIGHT",0,0)g.M:SetPoint("BOTTOMRIGHT",g.R,"BOTTOMLEFT",0,0) end
Layout(border,1)Layout(fill,math.max(2,inset))if btn.HookScript and not btn._msufSE3Hooked then btn._msufSE3Hooked=true btn:HookScript("OnSizeChanged",function() Layout(border,1);Layout(fill,math.max(2,inset)) end
)end
...
local font,size,flags=fs:GetFont()if not font or not size then return end
flags=flags or""if flags:find("ITALIC")then return end
...
local MSUF_ShowReloadRecommendedPopup local function MSUF_ApplyPreset(presetName) local preset=MSUF_PRESETS and MSUF_PRESETS[presetName]if type(preset)~="table"then print("|cffff3333MSUF:|r Preset not found: "..tostring(presetName)) return end
...
return else print("|cffff3333MSUF:|r Cannot load this preset (MSUF_ImportFromString missing).")end
end
end
...
local function MSUF_GetPresetNames() local names={}if type(MSUF_PRESETS)~="table"then return names end
...
if not StaticPopupDialogs["MSUF_LOAD_PRESET_CONFIRM"]then StaticPopupDialogs["MSUF_LOAD_PRESET_CONFIRM"]={text="Load preset: %s?\n\nThis will overwrite your CURRENT active profile settings.",button1=YES,button2=NO,timeout=0,whileDead=1,hideOnEscape=1,preferredIndex=3,OnAccept=function() if MSUF_PENDING_PRESET then MSUF_ApplyPreset(MSUF_PENDING_PRESET)end
...
local MSUF_PENDING_RELOAD_RECOMMEND_LABEL=nil MSUF_ShowReloadRecommendedPopup=function(label) if InCombatLockdown and InCombatLockdown()then if type(MSUF_Print)=="function"then MSUF_Print("Reload recommended (cannot show popup in combat).")else print("|cffffaa00MSUF:|r Reload recommended (cannot show popup in combat).")end
return end
...
if not StaticPopupDialogs["MSUF_RELOAD_RECOMMENDED"]then StaticPopupDialogs["MSUF_RELOAD_RECOMMENDED"]={text="MSUF recommends reloading the UI to ensure all changes apply correctly.\n\nApply: %s\n\nReload now?",button1="Reload",button2="Not now",timeout=0,whileDead=1,hideOnEscape=1,preferredIndex=3,OnAccept=function() MSUF_PENDING_RELOAD_RECOMMEND_LABEL=nil if type(ReloadUI)=="function"then ReloadUI()end
end
...
local MSUF_PENDING_RELOAD_LABEL=nil;local MSUF_PENDING_RELOAD_FN=nil local function MSUF_ShowReloadConfirm(label,fn) if InCombatLockdown and InCombatLockdown()then MSUF_Print("Cannot reload UI in combat.") return end
MSUF_PENDING_RELOAD_LABEL=tostring(label or"")MSUF_PENDING_RELOAD_FN=fn if not StaticPopupDialogs["MSUF_RELOAD_UI_CONFIRM"]then StaticPopupDialogs["MSUF_RELOAD_UI_CONFIRM"]={text="Reload UI now?\n\nThis will apply: %s",button1=YES,button2=NO,timeout=0,whileDead=1,hideOnEscape=1,preferredIndex=3,OnAccept=function() local f=MSUF_PENDING_RELOAD_FN;MSUF_PENDING_RELOAD_FN=nil MSUF_PENDING_RELOAD_LABEL=nil if type(f)=="function"then pcall(f)end
end
,OnCancel=function() MSUF_PENDING_RELOAD_FN=nil;MSUF_PENDING_RELOAD_LABEL=nil end
,}end
...
)f._msufEditBox=eb local ok=CreateFrame("Button",nil,f,"UIPanelButtonTemplate")ok:SetSize(120,24)ok:SetPoint("BOTTOM",f,"BOTTOM",0,12)ok:SetText(OKAY)ok:SetScript("OnClick",function() f:Hide() end
)if type(MSUF_SkinButton)=="function"then MSUF_SkinButton(ok)end
f:SetScript("OnShow",function(self) if self._msufTitleFS then self._msufTitleFS:SetText(self._msufTitle or"Link")end
...
)f:SetScript("OnHide",function(self) if self._msufEditBox then self._msufEditBox:SetText("");self._msufEditBox:ClearFocus()end
self._msufTitle=nil;self._msufUrl=nil end
)f:Hide()MSUF_CopyLinkPopup=f return f end
local function MSUF_ShowCopyLink(title,url) local f=MSUF_EnsureCopyLinkPopup()f._msufTitle=tostring(title or"Link")f._msufUrl=tostring(url or"")f:ClearAllPoints()f:SetPoint("CENTER",UIParent,"CENTER",0,0)f:Show()if f.Raise then f:Raise()end
end
local MIRROR_PANEL_SCALE=1.00;local MENU_FONT_BUMP=3;local MIRRORED_PANEL_FONT_BUMP=1;local MSUF_MIRROR_MAIN_CROP_Y=42;local MSUF_MIRROR_DEEPLINK_CROP_Y=96 local function MSUF_ApplyMidnightBackdrop(frame,alphaOverride) if not frame or not frame.SetBackdrop then return end
...
local function MSUF_SkinTitle(fs) if fs and fs.SetTextColor then fs:SetTextColor(MSUF_THEME.titleR,MSUF_THEME.titleG,MSUF_THEME.titleB,MSUF_THEME.titleA)end
end
local function MSUF_SkinText(fs) if fs and fs.SetTextColor then fs:SetTextColor(MSUF_THEME.textR,MSUF_THEME.textG,MSUF_THEME.textB,MSUF_THEME.textA)end
end
local function MSUF_SkinMuted(fs) if fs and fs.SetTextColor then fs:SetTextColor(MSUF_THEME.textR*0.80,MSUF_THEME.textG*0.80,MSUF_THEME.textB*0.80,0.85)end
end
local function MSUF_EnsureNavActiveOverlay(btn,fillGroup) if not(btn and btn.CreateTexture and fillGroup and fillGroup.L and fillGroup.M and fillGroup.R)then return nil end
...
g.SetVertexColor=function(self,r,gg,b,a) for i=1,#self._msufSEParts do local t=self._msufSEParts[i]if t and t.SetVertexColor then t:SetVertexColor(r,gg,b,a)end
end
end
g.Hide=function(self) for i=1,#self._msufSEParts do local t=self._msufSEParts[i]if t and t.Hide then t:Hide()end
end
...
g.L:SetAllPoints(fillGroup.L)g.M:SetAllPoints(fillGroup.M)g.R:SetAllPoints(fillGroup.R)g:Hide()btn._msufNavActive3=g return g end
...
if btn.HookScript and not btn.__msufBtnShowFix then btn.__msufBtnShowFix=true btn:HookScript("OnShow",function(self) if self.SetAlpha then self:SetAlpha(1)end
...
if btn.HookScript and not btn.__msufBtnHideFix then btn.__msufBtnHideFix=true btn:HookScript("OnHide",function(self) if self.UnlockHighlight then pcall(self.UnlockHighlight,self)end
...
local oldEnter=btn:GetScript("OnEnter");local oldLeave=btn:GetScript("OnLeave");local oldDown=btn:GetScript("OnMouseDown");local oldUp=btn:GetScript("OnMouseUp");local oldDis=btn:GetScript("OnDisable")local oldEn=btn:GetScript("OnEnable")local function ApplyBG(self,col) if not self or type(col)~="table"then return end
local bg=self._msufBtnBG if bg and bg.SetVertexColor then bg:SetVertexColor(col[1],col[2],col[3],col[4])end
end
btn:SetScript("OnEnter",function(self,...) ApplyBG(self,self._msufBtnBG_hover or self._msufBtnBG_base)if oldEnter then pcall(oldEnter,self,...)end
end
)btn:SetScript("OnLeave",function(self,...) ApplyBG(self,self._msufBtnBG_base)if oldLeave then pcall(oldLeave,self,...)end
end
)btn:SetScript("OnMouseDown",function(self,...) ApplyBG(self,self._msufBtnBG_pressed or self._msufBtnBG_hover)if oldDown then pcall(oldDown,self,...)end
end
)btn:SetScript("OnMouseUp",function(self,...) if self.IsMouseOver and self:IsMouseOver()then ApplyBG(self,self._msufBtnBG_hover or self._msufBtnBG_base)else ApplyBG(self,self._msufBtnBG_base)end
if oldUp then pcall(oldUp,self,...)end
end
)btn:SetScript("OnDisable",function(self,...) if self._msufBtnBG then self._msufBtnBG:SetVertexColor(0.07,0.07,0.08,0.65)end
...
)btn:SetScript("OnEnable",function(self,...) if self._msufBtnBG then self._msufBtnBG:SetVertexColor(0.09,0.10,0.12,0.92)end
...
local regions={b:GetRegions()}for i=1,#regions do local r=regions[i]if r and r.GetObjectType and r:GetObjectType()=="Texture"and r.GetTexture then local ok,tex=pcall(r.GetTexture,r)if ok and type(tex)=="string"then if tex:find("UI%-Panel%-Button")or tex:find("UIPanelButton")then return true end
end
end
end
return false end
local function MaybeFixCheckLabel(cb) local fs=cb and(cb.Text or cb.text or(cb.GetFontString and cb:GetFontString()))if not(fs and fs.GetTextColor and fs.SetTextColor)then return end
...
btn._msufApplyNavState=function(self,activeState,hovered) if self._msufNavActive3 then if activeState then self._msufNavActive3:Show()else self._msufNavActive3:Hide()end
end
...
local oldEnter=btn:GetScript("OnEnter");local oldLeave=btn:GetScript("OnLeave")btn:SetScript("OnEnter",function(self,...) if self._msufApplyNavState then self:_msufApplyNavState(self._msufNavIsActive,true)end
if oldEnter then pcall(oldEnter,self,...)end
end
)btn:SetScript("OnLeave",function(self,...) if self._msufApplyNavState then self:_msufApplyNavState(self._msufNavIsActive,false)end
...
local oldEnter=btn:GetScript("OnEnter");local oldLeave=btn:GetScript("OnLeave")btn:SetScript("OnEnter",function(self,...) if self._msufDashHover then self._msufDashHover._msufHovering=true if not self._msufDashIsSelected then self._msufDashHover:Show()end
end
if oldEnter then pcall(oldEnter,self,...)end
end
)btn:SetScript("OnLeave",function(self,...) if self._msufDashHover then self._msufDashHover._msufHovering=false;self._msufDashHover:Hide()end
if oldLeave then pcall(oldLeave,self,...)end
end
)local oldDown=btn:GetScript("OnMouseDown");local oldUp=btn:GetScript("OnMouseUp")btn:SetScript("OnMouseDown",function(self,...) if self._msufDashDown then self._msufDashDown:Show()end
if oldDown then pcall(oldDown,self,...)end
end
)btn:SetScript("OnMouseUp",function(self,...) if self._msufDashDown then self._msufDashDown:Hide()end
if oldUp then pcall(oldUp,self,...)end
end
)local oldDisable=btn:GetScript("OnDisable");local oldEnable=btn:GetScript("OnEnable")btn:SetScript("OnDisable",function(self,...) if self._msufDashBG then self._msufDashBG:SetVertexColor(0.06,0.06,0.07,0.65)end
...
)btn:SetScript("OnEnable",function(self,...) if self._msufDashBG then self._msufDashBG:SetVertexColor(0.08,0.09,0.11,0.92)end
...
ProcessChunk() end
local function MSUF_ApplyFontBumpToFrame(root,bump) bump=tonumber(bump or 0)or 0 if bump==0 or not root then return end
...
root.__MSUF_FontBumpInProgress=true root.__MSUF_FontBumpPending=nil local queue,qIndex={root},1;local visited={}local function bumpFont(obj) if not obj or not obj.GetFont or not obj.SetFont then return end
local ok,font,size,flags=pcall(obj.GetFont,obj)if not ok or not font or not size then return end
...
local function enqueue(child) if not child or visited[child]then return end
visited[child]=true queue[#queue+1]=child end
visited[root]=true local function processChunk() local t0=debugprofilestop();local budgetMs=6.0 while qIndex<=#queue do local frame=queue[qIndex]qIndex=qIndex+1 if frame then bumpFont(frame)if frame.EnumerateRegions then for reg in frame:EnumerateRegions()do bumpFont(reg)end
...
root.__MSUF_FontBumpApplied=bump root.__MSUF_FontBumpInProgress=nil local pending=root.__MSUF_FontBumpPending root.__MSUF_FontBumpPending=nil if pending and pending~=bump then C_Timer.After(0,function() if root and root.IsObjectType and root:IsObjectType("Frame")then MSUF_ApplyFontBumpToFrame(root,pending)end
end
)end
end
C_Timer.After(0,processChunk) end
local function MSUF_CollectMsufScaleFrames() local frames,seen={},{}local function add(f) if not f or seen[f]then return end
...
add(_G.MSUF_PlayerCastbar)add(_G.MSUF_TargetCastbar)add(_G.MSUF_FocusCastbar)add(_G.MSUF_PlayerCastbarPreview)add(_G.MSUF_TargetCastbarPreview)add(_G.MSUF_FocusCastbarPreview)add(_G.MSUF_BossCastbar)add(_G.MSUF_BossCastbarPreview) return frames end
local function MSUF_GetSavedMsufScale() local g=MSUF_EnsureGeneral()if not g then return 1.0 end
local v=tonumber(g.msufUiScale)if not v then v=tonumber(g.uiScale)end
if not v then v=1.0 end
return clamp(v,0.6,1.4) end
local function MSUF_SetSavedMsufScale(v) local g=MSUF_EnsureGeneral()if not g then return end
v=tonumber(v)or 1.0 g.msufUiScale=clamp(v,0.6,1.4) end
local function MSUF_IsScalingDisabled() local g=MSUF_EnsureGeneral and MSUF_EnsureGeneral()or nil return (g and g.disableScaling)and true or false end
local _MSUF_pendingMsufScale;local _MSUF_pendingGlobalScale;local _MSUF_pendingDisableScaling;local _MSUF_pendingReloadOnScalingOff;local _MSUF_scaleApplyWatcher local MSUF_EnsureScaleApplyAfterCombat local function MSUF_ApplyMsufScale(scale,opts) if MSUF_IsScalingDisabled()and not(opts and opts.ignoreDisable)then return end
scale=tonumber(scale)if not scale then return end
...
local function MSUF_EnforceUIParentScale(scale) scale=tonumber(scale)if not scale or scale<=0 then return end
scale=clamp(scale,0.3,2.0)if not(UIParent and UIParent.SetScale)then return end
...
local function MSUF_ScheduleUIParentNudges(scale) if MSUF_IsScalingDisabled()then return end
if not(C_Timer and C_Timer.After)then return end
scale=tonumber(scale)if not scale or scale<=0 then return end
local want=scale local function nudge() if InCombatLockdown and InCombatLockdown()then _MSUF_pendingGlobalScale=want if MSUF_EnsureScaleApplyAfterCombat then MSUF_EnsureScaleApplyAfterCombat()end
return end
MSUF_EnforceUIParentScale(want) end
C_Timer.After(0.05,nudge)C_Timer.After(0.25,nudge)C_Timer.After(0.60,nudge) end
local function MSUF_SetGlobalUiScale(scale,silent,opts) opts=opts or{}if MSUF_IsScalingDisabled()and not opts.ignoreDisable then return end
local applyCVars=(opts.applyCVars~=false)and true or false scale=tonumber(scale)if not scale or scale<=0 then return end
...
local f=CreateFrame("Frame")_MSUF_scaleApplyWatcher=f f:RegisterEvent("PLAYER_REGEN_ENABLED")f:SetScript("OnEvent",function() if InCombatLockdown and InCombatLockdown()then return end
...
if _MSUF_pendingReloadOnScalingOff then _MSUF_pendingReloadOnScalingOff=nil if type(ReloadUI)=="function"then ReloadUI(); return end
end
...
_G.MSUF_SetScalingDisabled=MSUF_SetScalingDisabled local function MSUF_SaveGlobalPreset(preset,scale) local g=MSUF_EnsureGeneral()if not g then return end
...
local want=MSUF_GetDesiredGlobalScaleFromDB()want=tonumber(want)if not want or want<=0 then return end
...
return end
MSUF_EnforceUIParentScale(want)if C_Timer and C_Timer.After then C_Timer.After(0,function() MSUF_EnforceUIParentScale(want) end
...
if _G and type(_G.MSUF_IsEditModeActive)=="function"then local ok,res=pcall(_G.MSUF_IsEditModeActive)if ok then return res and true or false end
end
...
if _G and type(_G.MSUF_SetMSUFEditModeDirect)=="function"then hooksecurefunc("MSUF_SetMSUFEditModeDirect",function(active) local win=_G and _G.MSUF_StandaloneOptionsWindow;local b=win and win._msufDashEditBtn if b and b._msufSetSelected then b:_msufSetSelected(active)end
end
...
local msufMinus,msufPlus;local pmDefs={{text="-",w=22,h=18,skinFn=MSUF_SkinDashboardButton,tipTitle=L["Decrease UI Scale"],tipBody="Decreases the global UI scale (UIParent).",onClick=function() if msufSlider and msufSlider.GetValue then msufSlider:SetValue((msufSlider:GetValue()or 100)-1)end
end
,},{text="+",w=22,h=18,skinFn=MSUF_SkinDashboardButton,tipTitle=L["Increase UI Scale"],tipBody="Increases the global UI scale (UIParent).",onClick=function() if msufSlider and msufSlider.GetValue then msufSlider:SetValue((msufSlider:GetValue()or 100)+1)end
end
,},}local pmRow=MSUF_BuildButtonRow(parent,msufSlider,"TOPRIGHT","BOTTOMRIGHT",0,-6,pmDefs,8)msufMinus,msufPlus=pmRow[1],pmRow[2]local resetW=120;local offW=180;local msufReset,msufOff;local row=MSUF_BuildButtonRow(parent,msufSlider,"TOPLEFT","BOTTOMLEFT",0,-6,{{text=L["Reset"],w=resetW,h=18,skinFn=MSUF_SkinDashboardButton,tipTitle=L["Reset UI Scale"],tipBody=L["Resets the global UI scale back to 100% (1.0) and marks it as Custom preset."],onClick=function() do local g=MSUF_EnsureGeneral and MSUF_EnsureGeneral()or nil if g then g.disableScaling=false end
...
if MSUF_AddTooltip then MSUF_AddTooltip(msufSlider,L["UI Scale (Global)"],L["Scales the entire WoW UI (UIParent). 100% = 1.0, 10% = 0.10. This sets the preset to Custom."])end
...
local cur=MSUF_GetCurrentGlobalUiScale()if cur then globalCur:SetText(string.format(L["Current: %.3f"],cur))else globalCur:SetText("Current: ?")end
...
local function MSUF_EnsureMainOptionsPanelBuilt() MSUF_SafeCall(EnsureDB)MSUF_SafeCall(_G and _G.MSUF_RegisterOptionsCategoryLazy)MSUF_SafeCall(_G and _G.CreateOptionsPanel)local p=_G and _G.MSUF_OptionsPanel if not p then return nil end
MSUF_ShowHideForLazy(p,"__MSUF_FullBuilt") return p end
local function MSUF_GetMainSettingsCategory() return (_G and _G.MSUF_SettingsCategory)or(ns and ns.MSUF_MainCategory) end
local function MSUF_EnsureMainSettingsCategory() local cat=MSUF_GetMainSettingsCategory()if not cat then MSUF_SafeCall(_G and _G.MSUF_RegisterOptionsCategoryLazy)cat=MSUF_GetMainSettingsCategory()end
return cat end
local SETTINGS_PANEL_DEFS={colors={full="MSUF_RegisterColorsOptions_Full",fallback="MSUF_RegisterColorsOptions",globals={"MSUF_ColorsPanel","MSUF_ColorsOptionsPanel"},builtKey="__MSUF_ColorsBuilt",},auras2={full="MSUF_RegisterAurasOptions_Full",fallback="MSUF_RegisterAurasOptions",globals={"MSUF_AurasPanel","MSUF_AurasOptionsPanel"},builtKey="__MSUF_AurasBuilt",},gameplay={full="MSUF_RegisterGameplayOptions_Full",fallback="MSUF_RegisterGameplayOptions",globals={"MSUF_GameplayPanel","MSUF_GameplayOptionsPanel"},builtKey="__MSUF_GameplayBuilt",},}local function MSUF_FindFirstGlobal(nameList) if not _G or type(nameList)~="table"then return nil end
for i=1,#nameList do local k=nameList[i];local obj=_G[k]if obj then return obj end
end
return nil end
local function MSUF_EnsureSubOptionsPanelBuilt(kind) local def=SETTINGS_PANEL_DEFS[kind]if not def then return nil end
local cat=MSUF_EnsureMainSettingsCategory()if not cat then return nil end
...
local p=MSUF_FindFirstGlobal(def.globals)MSUF_ShowHideForLazy(p,def.builtKey) return p end
...
local title=p:CreateFontString(nil,"OVERLAY","GameFontNormalLarge")title:SetPoint("TOPLEFT",12,-12)title:SetText(L["Modules"])local sub=p:CreateFontString(nil,"OVERLAY","GameFontNormal")sub:SetPoint("TOPLEFT",title,"BOTTOMLEFT",0,-6)sub:SetText(L["Optional MSUF modules and UI styling (MSUF only)."])if type(_G.MSUF_ApplyMidnightBackdrop)=="function"then pcall(_G.MSUF_ApplyMidnightBackdrop,p,0.96)end
...
local cb=CreateFrame("CheckButton",nil,p,"UICheckButtonTemplate")cb:SetPoint("TOPLEFT",sub,"BOTTOMLEFT",0,-14)if cb.Text then cb.Text:SetText(L["Enable MSUF Style"])if type(_G.MSUF_SkinText)=="function"then pcall(_G.MSUF_SkinText,cb.Text)end
end
local note=p:CreateFontString(nil,"OVERLAY","GameFontDisableSmall")note:SetPoint("TOPLEFT",cb,"BOTTOMLEFT",28,-6)note:SetText(L["Disabling may require /reload to fully remove existing styling."])if type(_G.MSUF_SkinMuted)=="function"then pcall(_G.MSUF_SkinMuted,note)end
local function GetEnabled() if type(_G.MSUF_StyleIsEnabled)=="function"then local ok,v=pcall(_G.MSUF_StyleIsEnabled)if ok then return v and true or false end
end
if _G.MSUF_DB and _G.MSUF_DB.general then return _G.MSUF_DB.general.styleEnabled~=false end
return true end
local function SetEnabled(v) if type(_G.MSUF_SetStyleEnabled)=="function"then pcall(_G.MSUF_SetStyleEnabled,v and true or false)else if _G.MSUF_DB and _G.MSUF_DB.general then _G.MSUF_DB.general.styleEnabled=v and true or false end
end
end
...
)local rb=CreateFrame("CheckButton",nil,p,"UICheckButtonTemplate")rb:SetPoint("TOPLEFT",note,"BOTTOMLEFT",-28,-12)if rb.Text then rb.Text:SetText(L["Rounded unitframes"])if type(_G.MSUF_SkinText)=="function"then pcall(_G.MSUF_SkinText,rb.Text)end
end
...
local function MSUF_SelectCastbarSubPage(unitKey) if type(_G and _G.MSUF_SetActiveCastbarSubPage)=="function"then pcall(_G.MSUF_SetActiveCastbarSubPage,unitKey)elseif type(MSUF_SetActiveCastbarSubPage)=="function"then pcall(MSUF_SetActiveCastbarSubPage,unitKey)end
...
},opt_colors={title="MSUF Colors",build=MSUF_EnsureColorsPanelBuilt},castbar={title="MSUF Castbar",build=MSUF_EnsureMainOptionsPanelBuilt,select=function(subkey) MSUF_SelectMainOptionsKey("castbar");if subkey and subkey~=""then MSUF_SelectCastbarSubPage(subkey)end
end
...
},colors={title="MSUF Colors",build=MSUF_EnsureColorsPanelBuilt},gameplay={title="MSUF Gameplay",build=MSUF_EnsureGameplayPanelBuilt},modules={title="MSUF Modules",build=MSUF_EnsureModulesPanelBuilt}}local function MSUF_GetMirrorPageInfo(key) return MIRROR_PAGES and key and MIRROR_PAGES[key]or nil end
local function MSUF_NormalizeMirrorKey(key,allowHome) key=key or(allowHome and"home"or"main")if allowHome and key=="home"then return"home"end
local info=MSUF_GetMirrorPageInfo(key)if info and key~="home"then return key end
return allowHome and"home"or"main" end
local function MSUF_GetPanelForKey(key) key=MSUF_NormalizeMirrorKey(key,false)local info=MSUF_GetMirrorPageInfo(key)or MSUF_GetMirrorPageInfo("main");local builder=info and info.build if type(builder)=="function"then return builder() end
return nil end
...
local tl=string.lower(t)return string.find(tl,"midnight simple unit frames",1,true)or string.find(tl,"beta version",1,true)or string.find(tl,"early version",1,true)or string.find(tl,"thank you for using",1,true) end
...
if not panel.__MSUF_MirrorWhiteHooked and panel.HookScript then panel.__MSUF_MirrorWhiteHooked=true panel:HookScript("OnShow",function() MSUF_ApplyWhiteTextToFrame(panel);MSUF_ApplyFontBumpToFrame(panel,MIRRORED_PANEL_FONT_BUMP)MSUF_ApplyMidnightControlsToFrame(panel) end
)end
if panel.LoadFromDB then pcall(panel.LoadFromDB,panel)elseif panel.Refresh then pcall(panel.Refresh,panel)end
MSUF_ApplyWhiteTextToFrame(panel)MSUF_ApplyFontBumpToFrame(panel,MIRRORED_PANEL_FONT_BUMP)MSUF_ApplyMidnightControlsToFrame(panel) end
local function MSUF_Standalone_SetCastbarTopButtonsHidden(hidden) local panel=_G and _G.MSUF_OptionsPanel;local editBtn=_G and _G["MSUF_CastbarEditModeButton"];local focusBtn=_G and _G["MSUF_CastbarFocusButton"]local function Capture(btn) if not btn or btn.__msufStandaloneAnchorBackup then return end
...
local function MSUF_Standalone_UpdateTitle(activeKey) if not(S.win and S.win._msufTitleFS and S.win._msufTitleFS.SetText)then return end
if activeKey=="home"then S.win._msufTitleFS:SetText("MSUF Menu") return end
local info=MSUF_GetMirrorPageInfo(activeKey)S.win._msufTitleFS:SetText((info and info.title)or"MSUF Menu") end
local function MSUF_Standalone_UpdateNav(activeKey) if not(S.win and S.win._msufNavButtons)then return end
...
local function MSUF_Standalone_ApplySelection(key,subkey,isCastbarKey) local info=MSUF_GetMirrorPageInfo(key);local sel=info and info.select;local wantSub=subkey if wantSub==nil and S and S.mirror then wantSub=S.mirror.pendingSubKey S.mirror.pendingSubKey=nil end
...
local function MSUF_Standalone_AttachMirrorPanel(key) local panel=MSUF_GetPanelForKey(key)if panel and S.mirror and S.mirror.host then MSUF_AttachMirroredPanel(panel,S.mirror.host,key)MSUF_ApplyWhiteTextToFrame(panel)MSUF_ApplyFontBumpToFrame(panel,MIRRORED_PANEL_FONT_BUMP)if C_Timer and C_Timer.After then C_Timer.After(0,function() if panel and panel.__MSUF_MirrorActive then MSUF_ApplyWhiteTextToFrame(panel);MSUF_ApplyFontBumpToFrame(panel,MIRRORED_PANEL_FONT_BUMP)end
end
)end
end
return panel end
local function MSUF_Standalone_AfterAttachFixups(key,isCastbarKey) if not isCastbarKey then return end
MSUF_Standalone_SetCastbarTopButtonsHidden(true)if C_Timer and C_Timer.After then C_Timer.After(0,function() MSUF_Standalone_SetCastbarTopButtonsHidden(true) end
...
local p=S and S.mirror and S.mirror.currentPanel if p and p.HookScript and not p.__MSUF_FocusKickResizeHooked then p.__MSUF_FocusKickResizeHooked=true p:HookScript("OnSizeChanged",function() if S and S.mirror and MSUF_IsCastbarKey(S.mirror.currentKey)then local function repin() if S and S.mirror and MSUF_IsCastbarKey(S.mirror.currentKey)then MSUF_Standalone_SetCastbarTopButtonsHidden(true)end
end
...
local function MSUF_SwitchMirrorPage(key,subkey) key=MSUF_NormalizeMirrorKey(key,true)if key=="home"then if S.mirror and MSUF_IsCastbarKey(S.mirror.currentKey)and not MSUF_IsCastbarKey(key)then MSUF_Standalone_SetCastbarTopButtonsHidden(false)end
...
MSUF_Standalone_UpdateTitle("home")MSUF_Standalone_UpdateNav("home") return end
if S.mirror.homePanel and S.mirror.homePanel.Hide then S.mirror.homePanel:Hide()end
local isCastbarKey=MSUF_IsCastbarKey(key)if S.mirror.currentKey==key and S.mirror.currentPanel and S.mirror.currentPanel.IsShown and S.mirror.currentPanel:IsShown()then MSUF_Standalone_ApplySelection(key,subkey,isCastbarKey)MSUF_Standalone_UpdateTitle(key)MSUF_Standalone_UpdateNav(key) return end
...
if S.mirror.currentPanel then MSUF_DetachMirroredPanel(S.mirror.currentPanel)S.mirror.currentPanel=nil end
S.mirror.currentKey=key S.mirror.currentPanel=MSUF_Standalone_AttachMirrorPanel(key)MSUF_Standalone_ApplySelection(key,subkey,isCastbarKey)MSUF_Standalone_AfterAttachFixups(key,isCastbarKey)MSUF_Standalone_UpdateTitle(key)MSUF_Standalone_UpdateNav(key) end
...
local function MakeButton(label,w,onClick,isHeader,isChild) local b=UI_Button(navParent,tostring(label or""),w,btnH,"TOPLEFT",navParent,"TOPLEFT",0,0,onClick)MSUF_LeftJustifyButtonText(b,isChild and 10 or 12)MSUF_SkinNavButton(b,isHeader,isChild) return b end
local NAV={{type="leaf",key="home",label=L["Dashboard"]},{type="header",id="unitframes",label=L["Unit Frames"],defaultOpen=true,children={{key="uf_player",label=L["Player"]},{key="uf_target",label=L["Target"]},{key="uf_targettarget",label=L["Target of Target"]},{key="uf_focus",label=L["Focus"]},{key="uf_boss",label=L["Boss Frames"]},{key="uf_pet",label=L["Pet"]},}},{type="header",id="options",label=L["Options"],defaultOpen=true,children={{key="opt_bars",label=L["Bars"]},{key="opt_fonts",label=L["Fonts"]},{key="auras2",label=L["Auras 2.0"]},{key="opt_castbar",label=L["Castbar"]},{key="opt_misc",label=L["Miscellaneous"]},{key="opt_colors",label=L["Colors"]},}},{type="leaf",key="gameplay",label=L["Gameplay"]},{type="header",id="modules",label=L["Modules"],defaultOpen=false,children={{key="modules",label=L["Style"]},}},{type="leaf",key="profiles",label=L["Profiles"]},}local headerLabels={}for _,node in ipairs(NAV)do if node.type=="header"then headerLabels[node.id]=node.label end
end
local created={}for _,node in ipairs(NAV)do if node.type=="leaf"then local b=MakeButton(node.label,btnW,function() MSUF_SwitchMirrorPage(node.key) end
,false,false)out[node.key]=b table.insert(created,{kind="leaf",btn=b})elseif node.type=="header"then headers[node.id]=(headers[node.id]~=nil)and headers[node.id]or node.defaultOpen local b=MakeButton("+ "..node.label,btnW,function() headers[node.id]=not headers[node.id]if navParent._msufTreeReflow then navParent._msufTreeReflow()end
end
,true,false)out["hdr_"..node.id]=b table.insert(created,{kind="header",id=node.id,btn=b})local kids={}for _,ch in ipairs(node.children or{})do local w=math.max(40,btnW-indent)local cb=MakeButton(ch.label,w,function() MSUF_SwitchMirrorPage(ch.key) end
,false,true)out[ch.key]=cb table.insert(kids,cb)table.insert(created,{kind="child",id=node.id,btn=cb})end
end
end
local function Reflow() local yOfs=-padT if hasTitle then yOfs=-(padT+26)end
...
local function ApplyMinHeight() local navRail=navParent and navParent.GetParent and navParent:GetParent()if not navRail then return end
...
)f:SetScript("OnDragStop",function(self) self:StopMovingOrSizing()if MSUF_SaveWindowGeometry then MSUF_SaveWindowGeometry(self,self._msufGeomKey or"full")end
end
)MSUF_AttachManualResizeGrip(f)MSUF_ApplyMidnightBackdrop(f,1.0)local title=f:CreateFontString(nil,"OVERLAY","GameFontNormalLarge")title:SetPoint("TOPLEFT",12,-10)title:SetText("MSUF Options")MSUF_SkinTitle(title)f._msufTitleFS=title local close=UI_CloseButton(f,"TOPRIGHT",f,"TOPRIGHT",-4,-4)local function MSUF_SetPropagateKeyboardInputSafe(frame,enabled) if not frame or not frame.SetPropagateKeyboardInput then return end
if type(InCombatLockdown)=="function"and InCombatLockdown()then frame._msufPendingPropagateKeyboard=enabled return end
frame._msufPendingPropagateKeyboard=nil frame:SetPropagateKeyboardInput(enabled) end
if not f._msufPropagateRegenFrame then local rf=CreateFrame("Frame")rf:RegisterEvent("PLAYER_REGEN_ENABLED")rf:SetScript("OnEvent",function() if f and f._msufPendingPropagateKeyboard~=nil then if f:IsShown()then MSUF_SetPropagateKeyboardInputSafe(f,f._msufPendingPropagateKeyboard)else f._msufPendingPropagateKeyboard=nil end
end
end
...
return card end
local function DashToggleEditMode() if type(_G.MSUF_SetMSUFEditModeDirect)=="function"then local st=_G.MSUF_EditState;local nextActive=true if st and st.active~=nil then nextActive=not st.active end
...
local function MSUF_ShowResetPositionsConfirm() if InCombatLockdown and InCombatLockdown()then MSUF_Print("Cannot reset while in combat.") return end
-- === Slash Commands ===

if not StaticPopupDialogs["MSUF_RESET_POS_CONFIRM"]then StaticPopupDialogs["MSUF_RESET_POS_CONFIRM"]={text="Reset MSUF frame positions now?\n\nThis resets MSUF frame positions + visibility to defaults for the ACTIVE profile.",button1=YES,button2=NO,timeout=0,whileDead=1,hideOnEscape=1,preferredIndex=3,OnAccept=function() if _G.SlashCmdList and _G.SlashCmdList["MIDNIGHTSUF"]then pcall(_G.SlashCmdList["MIDNIGHTSUF"],"reset")else MSUF_Print("/msuf reset handler not found.")end
end
,}end
StaticPopup_Show("MSUF_RESET_POS_CONFIRM") end
local function MSUF_ShowFactoryResetConfirm() if InCombatLockdown and InCombatLockdown()then MSUF_Print("Cannot factory reset while in combat.") return end
if not StaticPopupDialogs["MSUF_FACTORY_RESET_CONFIRM"]then StaticPopupDialogs["MSUF_FACTORY_RESET_CONFIRM"]={text="FACTORY RESET MSUF now?\n\nThis deletes ALL MSUF profiles & settings for this account.\n\nThe UI will reload.",button1=YES,button2=NO,timeout=0,whileDead=1,hideOnEscape=1,preferredIndex=3,OnAccept=function() if type(_G.MSUF_DoFullReset)=="function"then pcall(_G.MSUF_DoFullReset); return end
...
local quick=CreateCard(colL,L["Quick Actions"])quick:SetHeight(206)local bigW=410;local bigH=26;local LQ=MSUF_LayoutColumn(quick,12,-34,bigH,8);local qx1,qy1=LQ:Row(bigH,8);local bEdit=UI_BtnTL(quick,L["Toggle Edit Mode"],bigW,bigH,qx1,qy1,DashToggleEditMode,L["Toggle Edit Mode"],L["Enter MSUF Edit Mode to drag frames and adjust positions."],MSUF_SkinDashboardButton)local win=_G and _G.MSUF_StandaloneOptionsWindow if win then win._msufDashEditBtn=bEdit end
if bEdit and bEdit._msufSetSelected then bEdit:_msufSetSelected(MSUF_IsMSUFEditModeActive())end
MSUF_TryHookEditModeForDashboard()local qx2,qy2=LQ:Row(bigH,10);local bReset=UI_BtnTL(quick,L["Reset Frame Positions"],bigW,bigH,qx2,qy2,MSUF_ShowResetPositionsConfirm,L["Reset Frame Positions"],L["Resets MSUF frame positions + visibility to defaults (active profile)."],MSUF_SkinDashboardButton);local smallH=22;local qx3,qy3=LQ:Row(smallH,0);local rowBtns=MSUF_BuildButtonRowTL(quick,qx3,qy3,{{text=L["Colors"],w=160,h=smallH,gap=10,skinFn=MSUF_SkinDashboardButton,onClick=function() MSUF_SwitchMirrorPage("colors") end
},{text=L["Gameplay"],w=118,h=smallH,skinFn=MSUF_SkinDashboardButton,onClick=function() MSUF_SwitchMirrorPage("gameplay") end
},},10)local bColors=rowBtns[1];local bGameplay=rowBtns[2];local profCard=CreateCard(colL,L["Profile"],quick,-12)profCard:SetHeight(92)local LP=MSUF_LayoutColumn(profCard,12,-34,18,6);local px,py=LP:Row(18,0);local profLabel=UI_TextTL(profCard,"GameFontHighlight",px,py,L["Active profile:"],MSUF_SkinText);local profValue=UI_Text(profCard,"GameFontHighlight","LEFT",profLabel,"RIGHT",8,0,((_G and _G.MSUF_ActiveProfile)or"Default"),MSUF_SkinTitle)home._msufProfileValue=profValue local bProfiles=UI_Btn(profCard,L["Open Profiles"],160,22,"TOPRIGHT",profCard,"TOPRIGHT",-12,-30,function() MSUF_SwitchMirrorPage("profiles") end
,nil,nil,MSUF_SkinDashboardButton)do local DISCORD_URL="https://discord.gg/JQnhZXnTAK";local discordRow=CreateFrame("Frame",nil,profCard)discordRow:SetPoint("TOPLEFT",profCard,"TOPLEFT",12,-56)discordRow:SetPoint("TOPRIGHT",profCard,"TOPRIGHT",-12,-56)discordRow:SetHeight(20)local discordLabel=discordRow:CreateFontString(nil,"OVERLAY","GameFontHighlight")discordLabel:SetPoint("LEFT",discordRow,"LEFT",0,0)discordLabel:SetText("Discord:")MSUF_SkinText(discordLabel)local bDiscordSelect=UI_Btn(discordRow,"Select",72,18,"RIGHT",discordRow,"RIGHT",0,0,function() if discordRow._msufDiscordBox and discordRow._msufDiscordBox.SetFocus then discordRow._msufDiscordBox:SetFocus()if discordRow._msufDiscordBox.HighlightText then discordRow._msufDiscordBox:HighlightText()end
end
end
...
local function AdvApplyState(open) if open then advToggle:SetText(L["Hide"])advHint:SetText("")advBody:Show()else advToggle:SetText(L["Show"])advHint:SetText(L["Hidden. Click Show to reveal slash commands + power tools."])advBody:Hide()end
end
...
local bLoadPreset=UI_Btn(presetsCard,L["Load preset"],240,24,"TOPLEFT",presetDrop,"BOTTOMLEFT",16,-6,function() local sel=presetsCard._msufSelectedPreset if not sel then MSUF_Print("Select a preset first."); return end
MSUF_ShowPresetConfirm(sel) end
,L["Load preset"],L["Applies the selected preset to your current active profile. This overwrites settings (export first if unsure)."],MSUF_SkinDashboardButton)local presetHint=presetsCard:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")presetHint:SetPoint("TOPLEFT",bLoadPreset,"BOTTOMLEFT",0,-4)presetHint:SetText(L["Overwrites your current active profile settings."])MSUF_SkinMuted(presetHint)do local KO_FI_URL="https://ko-fi.com/midnightsimpleunitframes#linkModal";local PAYPAL_URL="https://www.paypal.com/ncp/payment/H3N2P87S53KBQ";local GITHUB_URL="https://github.com/Mapkov2/MidnightSimpleUnitFrames";local ICON_DIR="Interface\\AddOns\\MidnightSimpleUnitFrames\\Media\\Masks\\";local supportLabel=presetsCard:CreateFontString(nil,"OVERLAY","GameFontNormal")supportLabel:SetPoint("BOTTOMLEFT",presetsCard,"BOTTOMLEFT",12,14)supportLabel:SetText("Support the MSUF Development:")supportLabel:SetTextColor(0.90,0.90,0.90)supportLabel:SetJustifyH("LEFT")supportLabel:SetJustifyV("MIDDLE")if MSUF_SkinMuted then pcall(MSUF_SkinMuted,supportLabel)end
local row=CreateFrame("Frame",nil,presetsCard)row:SetHeight(24)row:SetWidth(120)row:SetPoint("BOTTOMRIGHT",presetsCard,"BOTTOMRIGHT",-12,12)local function CreateIcon(texFile,size,tooltipTitle,tooltipText,onClick) local b=CreateFrame("Button",nil,row)b:SetSize(size,size)local t=b:CreateTexture(nil,"ARTWORK")t:SetAllPoints()t:SetTexture(ICON_DIR..texFile)local hl=b:CreateTexture(nil,"HIGHLIGHT")hl:SetAllPoints()hl:SetColorTexture(1,1,1,0.10)b:SetScript("OnClick",onClick)if MSUF_AddTooltip then MSUF_AddTooltip(b,tooltipTitle,tooltipText)else b:SetScript("OnEnter",function(self) if not GameTooltip then return end
...
GameTooltip:Show() end
)b:SetScript("OnLeave",function() if GameTooltip then GameTooltip:Hide()end
end
)end
return b end
local sz=22;local gap=7 local icons={{tex="PayPal.png",title="PayPal",tip="Click to copy the PayPal support link.",onClick=function() MSUF_ShowCopyLink("PayPal",PAYPAL_URL) end
...
local function MSUF_DashboardLayout() local wL=(colL and colL.GetWidth and colL:GetWidth())or 0;local wR=(colR and colR.GetWidth and colR:GetWidth())or 0 if wL<=0 or wR<=0 then return end
...
if not home.__MSUF_DashboardLayoutHooked then home.__MSUF_DashboardLayoutHooked=true if home.HookScript then home:HookScript("OnShow",function() if C_Timer and C_Timer.After then C_Timer.After(0,MSUF_DashboardLayout)else MSUF_DashboardLayout()end
end
)home:HookScript("OnSizeChanged",function() if C_Timer and C_Timer.After then C_Timer.After(0,MSUF_DashboardLayout)else MSUF_DashboardLayout()end
end
)end
end
if C_Timer and C_Timer.After then C_Timer.After(0,MSUF_DashboardLayout)else MSUF_DashboardLayout()end
local tint=f:CreateTexture(nil,"BORDER")tint:SetColorTexture(0.06,0.12,0.25,0.16)tint:SetPoint("TOPLEFT",host,"TOPLEFT",0,0)tint:SetPoint("BOTTOMRIGHT",host,"BOTTOMRIGHT",0,0)f._msufTint=tint f:SetScript("OnShow",function() S.mirror.host=f._msufMirrorClipHost or f._msufMirrorHost or content do local nav=f._msufNavRail if nav and nav._msufTreeHeaders then nav._msufTreeHeaders.unitframes=true;nav._msufTreeHeaders.options=true if nav._msufTreeReflow then nav._msufTreeReflow()end
end
end
if MSUF_PickSessionTip then MSUF_PickSessionTip()end
local startKey=f._msufInitialKey or"home";local startSubKey=f._msufInitialSubKey;f._msufInitialKey=nil f._msufInitialSubKey=nil;S.mirror.currentKey=startKey MSUF_SwitchMirrorPage(startKey,startSubKey) end
)f:SetScript("OnHide",function() MSUF_Standalone_SetCastbarTopButtonsHidden(false)if MSUF_SaveWindowGeometry then MSUF_SaveWindowGeometry(f,f._msufGeomKey or"full")end
if S.mirror.currentPanel then MSUF_DetachMirroredPanel(S.mirror.currentPanel);S.mirror.currentPanel=nil end
end
)f:Hide()S.win=f return f end
local function MSUF_ToggleOptionsWindow(key,subkey) local w=MSUF_CreateOptionsWindow()if w:IsShown()then if key and S.mirror.currentKey~=key then MSUF_SwitchMirrorPage(key,subkey) return end
...
local function MSUF_ShowOptionsWindow(key,subkey) local w=MSUF_CreateOptionsWindow()key=key or"home"if w.IsShown and w:IsShown()then if key and S and S.mirror and S.mirror.currentKey~=key then MSUF_SwitchMirrorPage(key,subkey)elseif key then MSUF_SwitchMirrorPage(key,subkey)end
return w end
w._msufInitialKey=key w._msufInitialSubKey=subkey if w.Show then w:Show()end
return w end
local function MSUF_HideOptionsWindow() if S and S.win and S.win.IsShown and S.win:IsShown()and S.win.Hide then S.win:Hide()end
end
_G.MSUF_ShowStandaloneOptionsWindow=MSUF_ShowOptionsWindow _G.MSUF_OpenStandaloneOptionsWindow=MSUF_ShowOptionsWindow _G.MSUF_HideStandaloneOptionsWindow=MSUF_HideOptionsWindow _G.MSUF_OpenPage=function(key,subkey) key=(key or"home")if type(key)=="string"then key=key:lower()else key="home"end
...
if(key~="home")and(type(info.build)=="function")and(not panel)then MSUF_ShowOptionsWindow("home") return false end
MSUF_ShowOptionsWindow(key,subkey) return true end
MSUF_ShowOptionsWindow("home") return false end
_G.MSUF_OpenOptionsMenu=function() if _G and _G.MSUF_OpenPage then _G.MSUF_OpenPage("home")else MSUF_ShowOptionsWindow("home")end
end
local function MSUF_InstallSlashHook() if _G.MSUF_SlashHooked then return end
if SlashCmdList and SlashCmdList["MIDNIGHTSUF"]then local original=SlashCmdList["MIDNIGHTSUF"]SlashCmdList["MIDNIGHTSUF"]=function(msg) local raw=msg or""raw=raw:gsub("^%s+","")local first,rest=raw:match("^(%S+)%s*(.-)%s*$")first=first and first:lower()or""rest=rest or""local function openKey(key) MSUF_ToggleOptionsWindow(key) end
...
)if C_Timer and C_Timer.After then C_Timer.After(0,function() if MSUF_IsScalingDisabled and MSUF_IsScalingDisabled()then return end
