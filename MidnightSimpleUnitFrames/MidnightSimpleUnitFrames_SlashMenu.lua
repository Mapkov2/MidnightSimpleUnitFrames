...
local MIRROR_PAGES={home={title="Midnight Simple Unitframes (Version 1.85r1)",nav="Dashboard",build=nil},main={title="MSUF Options",nav="Options",build=MSUF_EnsureMainOptionsPanelBuilt,select=MSUF_MirrorSelectMain},uf_player={title="MSUF Player",build=MSUF_EnsureMainOptionsPanelBuilt,select=function()MSUF_SelectMainOptionsKey("player")end
...
local adv=CreateCard(colL,"Advanced",profCard,-12)adv:SetPoint("BOTTOMLEFT",colL,"BOTTOMLEFT",0,0)adv:SetPoint("BOTTOMRIGHT",colL,"BOTTOMRIGHT",0,0)local advTitle=adv._msufTitle local advToggle=UI_Btn(adv,"Show",88,18,"TOPRIGHT",adv,"TOPRIGHT",-12,-8,function()end
...
local function AdvApplyState(open)if open then advToggle:SetText("Hide")advHint:SetText("")advBody:Show()else advToggle:SetText("Show")advHint:SetText("Hidden. Click Show to reveal slash commands + power tools.")advBody:Hide()end
end
...
local bLoadPreset=UI_Btn(presetsCard,"Load preset",240,24,"TOPLEFT",presetDrop,"BOTTOMLEFT",16,-6,function()local sel=presetsCard._msufSelectedPreset if not sel then MSUF_Print("Select a preset first.");return end
