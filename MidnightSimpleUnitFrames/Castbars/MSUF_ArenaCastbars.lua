...
local function ApplyArenaCastbarsEnabled()
    -- Every castbar settings refresh lands here. On a client without arena units
    -- (Classic Era, WoW Forever) the pool is never built, and the profile keeps
    -- its arena castbar backend instead of being rewritten to HIDE, so a profile
    -- taken to a client with arenas still shows its arena castbars.
    if not HAS_ARENA_UNITS then
