...
local function RefreshLocaleText()
    ...
    L_BOSS_CASTBAR_PREVIEW = Tr("Celestial Ruin")
...
local function MSUF_HideBlizzardPlayerCastbar()
    EnsureDB()
    local function PlayerUsesBlizzardCastbar()
...
local function MSUF_CreateCastbarPreviewFrame(kind, frameName, opts)
    if type(_G.MSUF_CreateCastbarPreviewFrame) == "function" then
        local f = _G.MSUF_CreateCastbarPreviewFrame(kind, frameName, opts)
        if f and f.SetScale then f:SetScale(MSUF_GetSavedMsufScale()) end
        return f
    end
...
if type(_G.MSUF_UpdateBossCastbarPreview) ~= "function" then
    ...
    function _G.MSUF_UpdateBossCastbarPreview()
        ...
        local bossEnabled = type(shouldUse) == "function" and shouldUse("boss", g) or (g and g.enableBossCastbar ~= false)
        if not g or not g.castbarPlayerPreviewEnabled or not bossEnabled then
            if _G.MSUF_BossCastbarPreview then
...
local function _BossPreviewActiveConfig()
    ...
    if type(shouldUse) == "function" then
        if not shouldUse("boss", g) then return nil end
    elseif g.enableBossCastbar == false then
        return nil
    end
    return g
end
...
local function MSUF_PositionCastbarPreviewUnit(unit)
    EnsureDB()
    local g = (MSUF_DB and MSUF_DB.general) or {}
    if _G.MSUF_UnitEditModeActive ~= true or not g.castbarPlayerPreviewEnabled then
        return false
    end

    unit = tostring(unit or "")
    if unit == "player" then
        local f = _G.MSUF_PlayerCastbarPreview or MSUF_CreatePlayerCastbarPreview()
        if f and type(MSUF_PositionPlayerCastbarPreview) == "function" then
            MSUF_PositionPlayerCastbarPreview()
            if f.Show then f:Show() end
            return true
        end
    elseif unit == "target" then
        local f = _G.MSUF_TargetCastbarPreview or MSUF_CreateTargetCastbarPreview()
        if f and type(MSUF_PositionTargetCastbarPreview) == "function" then
            MSUF_PositionTargetCastbarPreview()
            if f.Show then f:Show() end
            return true
        end
    elseif unit == "focus" then
        local f = _G.MSUF_FocusCastbarPreview or MSUF_CreateFocusCastbarPreview()
        if f and type(MSUF_PositionFocusCastbarPreview) == "function" then
            MSUF_PositionFocusCastbarPreview()
            if f.Show then f:Show() end
            return true
        end
    elseif unit == "boss" or unit:match("^boss%d*$") then
        if _G.MSUF_InCombat == true
            or (InCombatLockdown and InCombatLockdown())
            or (_G.UnitAffectingCombat and _G.UnitAffectingCombat("player")) then
            return false
        end
        if type(_G.MSUF_UpdateBossCastbarPreview) == "function" then
            _G.MSUF_UpdateBossCastbarPreview()
