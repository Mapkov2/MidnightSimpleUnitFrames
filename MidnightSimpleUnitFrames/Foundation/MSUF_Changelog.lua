            ...
            sections = {
                {
                    ...
                    bullets = {
                        ...
                        "Aura2 incremental updates: Empty UNIT_AURA payloads are ignored early, while valid incremental aura deltas are processed through the cached aura update path instead of forcing a full invalidation scan during coalesced update bursts.",
                        ...
                        "Group Frame range fade refresh: Range fade resyncs are now queued through one delayed refresh for relevant spell, talent, spec, trait, world-entry, and combat-state events instead of running extra out-of-combat polling work.",
                        "Power text renderer: Power text now reads and formats only the values required by the selected display mode. Raw/component diff guards also include mode, separator, and split settings to avoid stale skips after option changes.",
                        "Health percent text: HP percent text clearing now happens only when needed and uses the precomputed text-spec requirement instead of repeatedly resolving the percent mode in hot paths.",
                        "Interrupt Ready colors: Ready, cooldown, and outline colors now reuse cached ColorMixin objects per configured RGBA value instead of allocating new color objects during refreshes.",
                ...
                },
                {
                    title = "Bugfixes",
                    bullets = {
                        "Fixed friendly dispel / purge capability detection using the correct class and race tokens, so aura outline event gating no longer misdetects player capabilities.",
                        "Fixed profile exports falling back to raw Lua table strings when a dirty runtime profile contains non-serializable transient values. Normal successful exports keep the existing MSUF3: format.",
                        ...
                        "Fixed Group Frame highlight and outline resolution for the newer aggro/dispel outline mode settings, including global settings and per-group overrides.",
                        "Fixed Group Frame aura preview fallback settings for aggro and target highlight indicators so the preview matches runtime behavior more closely.",
                ...
                },
                {
                    ...
                    bullets = {
                        "Dashboard Wago Profiles now shows the bundled changelog from the previous release to the current build in a readable, collapsible in-game release notes panel, including beta builds.",
                    },
                },
                {
