--- Shared - Menu / Dashboard- Midnight styling helpers.
--- Goal
...
Style.ApplyBackdrop = function(frame, alphaOverride, thinBorder)
  if not Style.IsEnabled() then  return end
...
Style.ApplyToFrame = function(root)
  if not Style.IsEnabled() then  return end
...
do
  ...
  --- Public entry points (MSUF + globals) so other option panels can call it.
...
do
  ...
  MSUF.MSUF_CreateStyledDropdown = function(name, parent, spec)
  ...
  MSUF.MSUF_PeelDropdownDefaults = DD_DEFAULT_SPEC
  _G.MSUF_PeelDropdownDefaults = DD_DEFAULT_SPEC
  MSUF.MSUF_RevertDropdownTemplate = RevertPeelDropdownTemplate
  ...
  MSUF.MSUF_ReskinDropdownLists = SkinDropDownLists
