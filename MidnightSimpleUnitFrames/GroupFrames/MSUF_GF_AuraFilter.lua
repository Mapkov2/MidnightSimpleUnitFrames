...
local DECLASSIFIED_META = {
    ...
    { key = "HEALER_HOTS",     label = "Healer HoTs & Shields",
      tooltip = "All healer HoTs: Rejuv, Renew, Riptide, Atonement, Echo, Reversion, etc." },
...
------------------------------------------------------------------------
-- PERF: Pre-warm blacklist hash cache at PLAYER_LOGIN
--
-- Observed in Perfy trace: BuildBlacklistHash has MAX 17.58ms latency.
-- That single outlier is the FIRST call for a gcfg when the cache is
-- cold — ~14 categories × ~200 spells forces Lua table-growth + GC.
-- If that cold build happens during combat, players see a frame drop.
--
-- Fix: force-build the hashes for party.{buff,debuff,externals} and
-- raid.{buff,debuff,externals} once at PLAYER_LOGIN. Subsequent calls
-- from RenderGroup (212/sec during combat) always hit the cached path
-- and cost <1µs.
--
-- Zero feature risk: InvalidateBlacklistHash is still called normally
-- on config change. This only moves the ONE cold build into load time.
------------------------------------------------------------------------
local function PrewarmBlacklistHashes()
    local db = _G.MSUF_DB
    if type(db) ~= "table" then return end

    -- DB layout: MSUF_DB.gf_party and MSUF_DB.gf_raid (flat keys).
    -- Verified against GF.InvalidateConfCache in MSUF_GF_DB.lua.
    local roots = { db.gf_party, db.gf_raid }
    local groups = { "buff", "debuff", "externals" }
    for i = 1, #roots do
        local conf = roots[i]
        local auras = conf and conf.auras
        if type(auras) == "table" then
            for j = 1, #groups do
                local g = auras[groups[j]]
                if type(g) == "table" and type(g.blacklistCats) == "table" then
                    -- Build & cache. Return value intentionally ignored.
                    BuildBlacklistHash(g)
                end
            end
        end
    end
end
AuraFilter.PrewarmBlacklistHashes = PrewarmBlacklistHashes
...
do
    ...
    ef:SetScript("OnEvent", function(self)
        ...
        -- Defer one frame so GF_DB migrations and defaults are settled.
        if C_Timer and C_Timer.After then
            C_Timer.After(0, PrewarmBlacklistHashes)
        else
            PrewarmBlacklistHashes()
        end
    end)
end
