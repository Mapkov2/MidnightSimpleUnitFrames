...
local DEFAULT_TOOLTIP = { tooltipMode = "ALWAYS", tooltipModifier = "ALT" }

local APPLY_MASK = {
    Health = true, Power = true, Text = true, NameText = true,
    HealthText = true, PowerText = true, StatusIndicators = true,
    GroupAuraCache = true, GroupStatusRuntime = true, Prediction = true,
    Alpha = true, Auras = true,
    Borders = true, GroupRangeFade = true, GroupVisuals = true,
    GroupBlizzardAuras = true,
    GroupCornerIndicators = true, GroupSpellIndicators = true, GroupPrivateAuras = true,
}
...
local function TooltipConf(frame)
    local layout = frame and frame.MSUFSpec and frame.MSUFSpec.groupLayout
    if layout then
        return layout
    end
...
local function TooltipAllowed(conf)
    local mode = conf and (conf.tooltipMode or conf.mode) or "ALWAYS"
    if mode == "NEVER" then return false end
    if mode == "OOC" then return not InCombat() end
    if mode == "MODIFIER" then
        local mod = conf.tooltipModifier or conf.modifier or "ALT"
...
local function ShowTooltip(frame)
    ...
    if not TooltipAllowed(conf) then
        return
    end
...
local function HookButton(frame)
    ...
    frame:HookScript("OnEnter", function(self)
        ...
        local conf = TooltipConf(self)
        if not TooltipAllowed(conf) then return end
    ...
    frame:HookScript("OnLeave", function(self)
        ...
        self._msufGFTooltipToken = (self._msufGFTooltipToken or 0) + 1
        if UnitFrame_OnLeave then UnitFrame_OnLeave(self)
...
local function OnChildAttributeChanged(self, name, value)
    ...
    UF.OnUnitChanged(self, self.unit, value)
