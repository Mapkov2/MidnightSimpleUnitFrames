...
local function GradientScopeValue(kind, conf, general, key, fallback)
...
local function ResolveBarGradient(kind, conf, general, enabledKey)
    local left = GradientScopeValue(kind, conf, general, "gradientDirLeft", false) == true
    local right = GradientScopeValue(kind, conf, general, "gradientDirRight", false) == true
    local up = GradientScopeValue(kind, conf, general, "gradientDirUp", false) == true
    local down = GradientScopeValue(kind, conf, general, "gradientDirDown", false) == true
    if not (left or right or up or down) then
        local legacy = GradientScopeValue(kind, conf, general, "gradientDirection", "RIGHT")
    ...
    return {
        enabled = GradientScopeValue(kind, conf, general, enabledKey, false) == true,
        strength = Clamp01(GradientScopeValue(kind, conf, general, "gradientStrength", 0.45), 0.45),
        left = left,
        right = right,
        up = up,
        down = down,
...
local function CompileSpecUncached(kind, frame, unit, conf)
    ...
    return {
        ...
        power = {
            ...
            barGradient = ResolveBarGradient(kind, conf, general, "enablePowerGradient"),
        ...
        border = {
            ...
            r = Num(GroupScopeValue(kind, conf, general, "barOutlineColorR", conf.borderR or general and general.barBorderR), 0),
            g = Num(GroupScopeValue(kind, conf, general, "barOutlineColorG", conf.borderG or general and general.barBorderG), 0),
            b = Num(GroupScopeValue(kind, conf, general, "barOutlineColorB", conf.borderB or general and general.barBorderB), 0),
            a = Num(GroupScopeValue(kind, conf, general, "barOutlineColorA", conf.borderA or general and general.barBorderA), 1),
