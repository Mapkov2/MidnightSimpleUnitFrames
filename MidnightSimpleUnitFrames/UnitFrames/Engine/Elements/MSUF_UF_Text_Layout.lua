...
local ANCHOR_HASH = { TOPLEFT = 1, TOP = 2, TOPRIGHT = 3, LEFT = 4, CENTER = 5, RIGHT = 6, BOTTOMLEFT = 7, BOTTOM = 8, BOTTOMRIGHT = 9 }
...
local function TextApplySignature(spec, text)
    ...
    return concat({
        ...
        tostring(spec and spec.fontShadowX),
        tostring(spec and spec.fontShadowY),
...
function Text.Apply(frame, spec)
    ...
    if inlineEnabled then
        ...
        elseif AnchorInlineToNameClip(frame) then
            -- Separator is anchored to the visible clipped name edge.
        else
