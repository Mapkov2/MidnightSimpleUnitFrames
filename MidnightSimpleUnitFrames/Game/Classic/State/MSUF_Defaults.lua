...
local MSUF_DEFAULTS_SPEC = {}
for key, value in pairs(StateHelpers.DefaultsSpec) do MSUF_DEFAULTS_SPEC[key] = value end
MSUF_DEFAULTS_SPEC.statusPrefixes = {}
for i, prefix in ipairs(StateHelpers.DefaultsSpec.statusPrefixes) do
    MSUF_DEFAULTS_SPEC.statusPrefixes[i] = prefix
end
MSUF_DEFAULTS_SPEC.statusPrefixes[#MSUF_DEFAULTS_SPEC.statusPrefixes + 1] = "petHappinessIndicator"
...
--- 6.5-alpha18 to 6.5-beta3 saved the factory scope above without its final
--- re-materialize: every authored owner kept only its sparse tables under a
--- lane-owner marker that was already set, so no pass ever completed it, and
--- the Classic aura compile, which reads only the owner, anchored buffs
...
local function MSUF_Defaults_CreateFactoryProfile()
    local tbl = _G.MSUF_TryDecodeCompactString(MSUF_FACTORY_DEFAULT_PROFILE_COMPACT)
...
--- Every stored profile, not only the active one: the persisted fast path skips
--- the heavy pass for them, and an owner is repaired or kept by its own shape.
