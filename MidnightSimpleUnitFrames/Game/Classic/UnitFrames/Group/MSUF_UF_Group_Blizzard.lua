...
local function HidePartyFrames()
  -- CompactPartyFrame and the pooled PartyMemberFrames are descendants of
  -- PartyFrame on the supported Classic clients. Hide only that owner; touching individual unit buttons
...
---
--- The Classic Raid Manager is kept out of the hard-hide ownership pass so its
--- ready-check, marker and filter controls retain their native event ownership.
--- This layer controls its visible chrome without unregistering or reparenting it.
---
...
local function EnsureRaidManagerHooks(manager)
  ...
  --- Supported Classic branches retain the single legacy toggle button.
...
function GF.RestoreBlizzardGroupFrames()
  ...
  -- not safe on current protected CompactUnitFrames. Provider changes are applied by the clean native
