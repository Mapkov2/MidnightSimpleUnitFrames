...
modeBuilders.CONTINUOUS = function(E)
    ...
    --- MoP Eclipse power is signed (-max lunar .. +max solar). Keeping this
    --- in the Classic-selected mode avoids changing the Retail continuous
    --- path and still lets the native StatusBar represent both directions.
    local function UpdateSigned(powerType, maxPower)
        local rawCur = UnitPower("player", powerType)
        local rawMx = UnitPowerMax("player", powerType)
        local bar = CP.bars[1]
        if not bar then return end
