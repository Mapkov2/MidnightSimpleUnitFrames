...
local CPK = CPConst.CPK or { MODE = { NONE = 0, SEGMENTED = 1, FRACTIONAL = 2, RUNE_CD = 3, AURA_SEGMENTED = 4, AURA_SINGLE = 5, CONTINUOUS = 6, TIMER_BAR = 8, STAGGER = 9, IRONFUR = 10, SIGNED_CONTINUOUS = 11 }, SPEC = {}, SPELL = {}, BAL = {}, THRESH = {} }
...
local function GetClassPowerType()
    ...
    if clientProvider and type(clientProvider.Resolve) == "function" then
        ...
        if handled then return powerType, renderMode or CPK.MODE.NONE, isAuraPower == true end
    end
...
do
    ...
    if continuous and type(continuous.UpdateSigned) == "function" then CP.UpdateSignedContinuous = continuous.UpdateSigned end
...
local function CP_GetModeEventProfile(renderMode, powerType, isAuraPower)
    ...
    profile.targetChanged = provider and type(provider.NeedsTargetChanged) == "function"
        and provider.NeedsTargetChanged(powerType) == true or false
    return profile
end
