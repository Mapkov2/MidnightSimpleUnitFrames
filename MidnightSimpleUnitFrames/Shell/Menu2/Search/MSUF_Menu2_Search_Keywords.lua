...
Data.KEYWORDS = {
    ...
    auras3 = "aura style styling colors text stack stacks cooldown timer colors cooldown text stack count font size anchor offset safe warning urgent own buff own debuff group frame buffs debuffs cooldown swipe",
    auras3_buffs = "buff buffs aura style styling text stack stacks cooldown timer colors cooldown text stack count font size own buff group frame buff settings",
    auras3_debuffs = "debuff debuffs aura style styling text stack stacks cooldown timer colors cooldown text stack count font size own debuff group frame debuff settings",
    auras3_rendering = "aura style styling colors text stack stacks cooldown timer colors cooldown text stack count font size anchor offset safe warning urgent own buff own debuff group frame buffs debuffs cooldown swipe",
    ...
    opt_misc = "global style miscellaneous misc language localization localisation locale translation range fade range check range checker distance check out of range unit frame range check ui behavior tooltip tooltips unitframe tooltip group frame tooltip mouseover tooltip modifier tooltip combat settings general blizzard frames default frames hide blizzard disable blizzard minimap minimap icon target sounds version check startup notices menu behavior snap edge snap navigation icons nav icons navigation symbols nav symbols navi symbole navigationssymbole",
    ...
    gameplay = "gameplay combat timer combat state combat enter combat leave combat crosshair crosshair size thickness in range out of range melee range spell first dance first dance tracker rogue buff totem frame totems statue frame statue icons player totems click cast click cast clickthrough click-through focus target modifier mouseover interaction targeting spells mouse buttons keybind modifier ctrl shift alt fadenkreuz target sound target lost mouseover heal click casting",
