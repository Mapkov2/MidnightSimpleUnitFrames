...
local AURA_OPTIONS = {
    SCOPES = AURA_SCOPES,
    GROWTH = AURA_GROWTH,
    ROW_WRAP = AURA_ROW_WRAP,
    STACK_ANCHORS = AURA_STACK_ANCHORS,
...
local function AddTooltip(widget, title, body)
    return M.AddTooltip and M.AddTooltip(widget, title, body, {
        hook = true,
        labelHit = true,
        titleAsLine = true,
        bodyColor = { 0.72, 0.76, 0.86 },
    }) or widget
end
...
local function BuildAuras(ctx)
    ...
    local function AuraScopeLabel()
        local scopeKey = AuraScope()
        for i = 1, #AO.SCOPES do
            local spec = AO.SCOPES[i]
    ...
    local function ApplyUnitAuraEnabled()
        local a2 = AurasDB()
        if a2.enabled == false and type(_G.MSUF_Auras3_RefreshAll) == "function" then pcall(_G.MSUF_Auras3_RefreshAll) end
        ApplyAuras()
    end
    ...
    local function ResetAllAuraOverrides()
        local a2 = AurasDB()
        for i = 2, #AO.SCOPES do
            local key = AO.SCOPES[i].value
            local u = a2.perUnit and a2.perUnit[key]
    ...
    local scopeDrop = ValueDropdownAt(ctx, scopeCard, "Editing scope", 16, -50, AO.SCOPES, scopeDropdownW,
        AuraScope,
        function(value)
            if type(M.PersistMenuStateValue) == "function" then
    ...
    resetScope:SetScript("OnClick", function()
        M.CaptureHistory("Reset aura scope", "auras3:scope:reset", ResetSelectedScope)
    end)
    ...
    resetAll:SetScript("OnClick", function()
        M.CaptureHistory("Reset all aura overrides", "auras3:overrides:reset", ResetAllAuraOverrides)
    end)
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, layout, "Buff Growth", layoutCol1, -308, AO.GROWTH, layoutDropdownW,
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, layout, "Debuff Growth", layoutCol2, -308, AO.GROWTH, layoutDropdownW,
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, layout, "Buff wrap rows", layoutCol3, -308, AO.ROW_WRAP, layoutDropdownW,
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, layout, "Debuff wrap rows", layoutCol4, -308, AO.ROW_WRAP, layoutDropdownW,
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, layout, "Sort order", layoutCol3, -392, AO.SORT_ORDER, layoutDropdownW * 2 + layoutGap,
        function()
            local c = AuraCaps()
    ...
    Track(capsOverrideControls, ValueDropdownAt(ctx, filters, "Sort order", 380, -202, AO.SORT_ORDER, 270,
        function()
            local c = AuraCaps()
            if type(c.sortOrder) == "number" then return c.sortOrder end
            local f = AuraFilters()
    ...
    for i = 1, #AO.IGNORE_CATEGORIES do
        local spec = AO.IGNORE_CATEGORIES[i]
    ...
    for i = 1, #AO.REMINDERS do
        local spec = AO.REMINDERS[i]
        local col = (i <= 5) and 12 or 380
        local row = (i <= 5) and i or (i - 5)
    ...
    local grow = DropdownAt(ctx, reminders, "Grow Direction", 500, -200, AO.GROWTH, 190, AuraShared, "reminderGrowth", "RIGHT", function() MarkReminderDirty(); ApplyAuras() end)
...
M.RegisterPage("auras3", { title = "MSUF Unit Auras", build = BuildAuras, version = 8 })
