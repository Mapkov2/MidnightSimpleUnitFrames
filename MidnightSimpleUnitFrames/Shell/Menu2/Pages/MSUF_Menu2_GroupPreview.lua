...
local function CreateNativeGFPreview(parent, ctx)
    ...
    if type(create) == "function" then
        return create(parent, ctx, GroupPreview.OpenSection)
    end
    return nil
end
...
local function AddGFPreview(ctx, builder)
    ...
    local function RefreshThisPreview()
        ...
        if preview and preview.IsShown and preview:IsShown() then
            RequestPreviewRefresh(preview, "GROUP_PREVIEW_PAGE_REFRESH")
        end
    end

    M.SetCollapsibleRefreshState(body, RefreshThisPreview)
