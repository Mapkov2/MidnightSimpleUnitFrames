...
local function MSUF_SetScopePillColors(btn, bg, br, tr, hover)
    ...
    if btn._msufScopeFill then
        local r, g, b, a = min(bg[1] * mul, 1), min(bg[2] * mul, 1), min(bg[3] * mul, 1), bg[4] or 0.92
        if btn._msufScopeFill.SetVertexColor then btn._msufScopeFill:SetVertexColor(r, g, b, a)
        elseif btn._msufScopeFill.SetColorTexture then btn._msufScopeFill:SetColorTexture(r, g, b, a) end
    end
    if btn._msufScopeBorder then
        local r, g, b, a = min(br[1] * mul, 1), min(br[2] * mul, 1), min(br[3] * mul, 1), br[4] or 0.82
        if btn._msufScopeBorder.SetBackdropBorderColor then btn._msufScopeBorder:SetBackdropBorderColor(r, g, b, a)
        elseif btn._msufScopeBorder.SetVertexColor then btn._msufScopeBorder:SetVertexColor(r, g, b, a) end
    end
...
function _G.MSUF_SkinScopePillButton(btn, text, width, height, bg, br, tr)
    ...
    local nativeTex = btn.GetNormalTexture and btn:GetNormalTexture()
    if nativeTex and nativeTex.Hide then nativeTex:Hide() end
    nativeTex = btn.GetHighlightTexture and btn:GetHighlightTexture()
    if nativeTex and nativeTex.Hide then nativeTex:Hide() end
    nativeTex = btn.GetPushedTexture and btn:GetPushedTexture()
    if nativeTex and nativeTex.Hide then nativeTex:Hide() end
    nativeTex = btn.GetDisabledTexture and btn:GetDisabledTexture()
    if nativeTex and nativeTex.Hide then nativeTex:Hide() end
    ...
    if btn._msufBg then
        btn._msufScopeFill = btn._msufBg
    elseif not btn._msufScopeFill then
        local fill = btn:CreateTexture(nil, "BACKGROUND")
        fill:SetTexture(MSUF_TEX_SUPERELLIPSE)
        fill:SetPoint("TOPLEFT", btn, "TOPLEFT", 2, -2)
        fill:SetPoint("BOTTOMRIGHT", btn, "BOTTOMRIGHT", -2, 2)
        btn._msufScopeFill = fill
    end
    if btn._msufBorder then
        btn._msufScopeBorder = btn._msufBorder
    elseif not btn._msufScopeBorder then
        local border = btn:CreateTexture(nil, "BACKGROUND", nil, 0)
        border:SetTexture(MSUF_TEX_SUPERELLIPSE)
        border:SetAllPoints()
        btn._msufScopeBorder = border
    end
    local fs = btn._msufScopeLabel or (btn.GetFontString and btn:GetFontString())
    if not fs then
    ...
    if fs.SetShadowColor then fs:SetShadowColor(0, 0, 0, 0.55) end
