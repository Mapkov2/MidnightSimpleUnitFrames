...
function GF.BuildAuraOptionsSections(AddSection, SCheck, SSlider, SDropdown, K, TrackRefresh, MakeColorSwatch, OpenColorPicker, refreshFns)
    ...
    ----------------------------------------------------------------
    -- Section: Raid Debuffs (Priority single large debuff icon)
    ----------------------------------------------------------------
    do
        local ANCHOR9 = {
        ...
        SCheck({
            ...
            get = function(k) return RD().enabled == true end,
            set = function(k, v) RD().enabled = v; GF.RefreshVisuals() end,
        })
        ...
        local rdXSl = SSlider({
            ...
            formatText = function(v) return string.format("Offset X: %d", v) end,
        })
        ...
        local rdYSl = SSlider({
            ...
            formatText = function(v) return string.format("Offset Y: %d", v) end,
        })
        ...
        refreshFns[#refreshFns + 1] = function()
            -- Raid Debuffs section doesn't need dynamic refresh
        end
    end
