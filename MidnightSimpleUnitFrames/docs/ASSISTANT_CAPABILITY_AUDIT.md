...
- The offline registry harness loads `2626` settings and `127` actions.
- The strict UI matrix now reports `708` UI rows: `607` registered, `2` partial, `54` documented TODO, and `45` unmapped.
...
- The registry-backed parser fallback now uses a token candidate index before exact setting scoring, avoiding a full registry scan for every natural-language message while preserving the existing value, ambiguity, and safety gates.
...
| Metric | Count |
| --- | ---: |
| Registered settings | 2626 |
| Registered actions | 127 |
| Registered diagnostic actions | 13 |
| Registered navigation actions | 17 |
| Verified visible left-nav targets | 24 |
| UI matrix rows | 708 |
| UI matrix registered rows | 607 |
| UI matrix partial rows | 2 |
| UI matrix TODO rows | 54 |
| UI matrix unmapped rows | 45 |
| Non-aura unmapped rows | 0 |

Setting type counts: `{'boolean': 745, 'number': 1066, 'enum': 543, 'string': 90, 'color': 182}`

Action type counts: `{'auras': 6, 'classPower': 1, 'color': 16, 'configure': 3, 'copy': 2, 'diagnostic': 13, 'fonts': 2, 'gameplay': 3, 'globalBars': 5, 'history': 5, 'navigation': 17, 'preset': 2, 'preview': 3, 'profile': 19, 'reset': 14, 'setup': 14, 'support': 2}`
...
| Appearance / Class Resources | Registry coverage exists for class resource settings; current class-power page rows are registered, including natural movement, width-mode, opacity, prediction, Alternative Mana, quick setup, and diagnostics with numbered suggested fixes for stable setting-backed causes. |
...
- Parser performance pass: the generic registry fallback now prefilters candidates by indexed meaningful tokens. The broad `turn off ready check symbol for all group frames` path uses `203` candidate settings instead of scanning all `2626` settings; the isolated parse measured about `0.82s` in the offline harness.
