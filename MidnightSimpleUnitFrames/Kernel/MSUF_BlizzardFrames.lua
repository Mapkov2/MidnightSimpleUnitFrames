...
local function DisableBlizzardFrames()
    ...
    --- Flavor-owned suppression passes (for example the Classic legacy arena
    --- frames in Game/Classic/BlizzardFrames.lua). Each pass receives the
    --- shared HandleFrame and ShouldHideBlizzardUnitFrame helpers, so combat
    --- deferral and reparent hooks stay in this file. Mainline registers none.
    local passes = MSUF.BlizzardFrameSuppressionPasses
    if type(passes) == "table" then
        for i = 1, #passes do
            passes[i](HandleFrame, ShouldHideBlizzardUnitFrame)
        end
    end
