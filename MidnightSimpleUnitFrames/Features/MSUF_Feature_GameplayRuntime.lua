--- Features/MSUF_Feature_GameplayRuntime.lua
--- Feature-gated gameplay runtime.
---
--- Owns: combat timer, combat state text, combat crosshair range driver, First
--- Dance helper, and gameplay feature registration. Disabled features must
--- unregister timers/events and do no background work.
local _, MSUF = ...
MSUF = MSUF or {}
...
local MSUF_GetPlayerSpecID, _UpdateSubRogueCache = GameplayHelpers.GetPlayerSpecID, GameplayHelpers.UpdateSubRogueCache
local _MSUF_Clamp, _MSUF_RoundInt = GameplayHelpers.Clamp or _G._MSUF_Clamp, GameplayHelpers.RoundInt or _G._MSUF_RoundInt
local MSUF_Gameplay_IsTextInputFocused, MSUF_Gameplay_GetNudgeStep = GameplayHelpers.IsTextInputFocused, GameplayHelpers.GetNudgeStep
local MSUF_Gameplay_CheckpointHistory, MSUF_Gameplay_BeginHistory, MSUF_Gameplay_CommitHistory = GameplayHelpers.CheckpointHistory, GameplayHelpers.BeginHistory, GameplayHelpers.CommitHistory
local MSUF_Gameplay_SelectNudgeFrame, MSUF_Gameplay_EnableKeyboardNudge, MSUF_Gameplay_SetupArrowNudge = GameplayHelpers.SelectNudgeFrame, GameplayHelpers.EnableKeyboardNudge, GameplayHelpers.SetupArrowNudge
...
---
--- Gameplay config/default/font/color ownership moved to MSUF_Feature_GameplayConfig.lua
---
local EnsureGameplayDefaults = MSUF.MSUF_EnsureGameplayDefaults
local GetGameplayDBFast = MSUF.MSUF_GetGameplayDBFast
local GetGameplayFontSettings = MSUF.MSUF_GetGameplayFontSettings
local NormalizeRGB = MSUF.MSUF_NormalizeRGB or function(tbl, dr, dg, db)
...
local MSUF_ApplyCombatStateDynamicColor
...
local function MSUF_CombatTimerLoopStep()
    local loop = MSUF._MSUF_CombatTimerLoopActive
    if not loop then return end
    MSUF_Gameplay_TickCombatTimer()
    if MSUF._MSUF_CombatTimerLoopActive == loop and C_Timer and C_Timer.After then
        C_Timer.After(1.0, loop.step)
    elseif MSUF._MSUF_CombatTimerLoopActive == loop then
        MSUF._MSUF_CombatTimerLoopActive = nil
    end
end

local function _StartCombatTimerTick()
    if MSUF._MSUF_CombatTimerLoopActive then return end
    if not (C_Timer and C_Timer.After) then return end
    local loop = {}
    loop.step = function()
        if MSUF._MSUF_CombatTimerLoopActive == loop then
        ...
        _fdShadowDanceFrame:SetScript("OnEvent", function(_, _, _, _, spellID)
            if spellID == _SHADOW_DANCE_SPELL_ID then
                ...
                if not UnitAffectingCombat("player") then
                    ...
                    if gd and gd.enableFirstDanceTimer and MSUF._MSUF_StartFirstDanceWindow then
                        MSUF._MSUF_StartFirstDanceWindow()
                    end
                end
            end
        end)
...
local function EnsureFirstDanceFrame()
    ...
    if not MSUF._MSUF_FirstDanceModifierFrame then
        local mf = CreateFrame("Frame")
        MSUF._MSUF_FirstDanceModifierFrame = mf
...
local function _ApplyFirstDanceDisplayMode()
    ...
    if iconMode then
        ...
        if MSUF.MSUF_FirstDance_ApplyMasque then MSUF.MSUF_FirstDance_ApplyMasque() end
    else
...
do
    ...
    local function _FD_IsMasqueEnabled()
        local a2db = MSUF_DB and MSUF_DB.auras3
    ...
    function MSUF.MSUF_FirstDance_ApplyMasque()
        if not firstDanceFrame then return end
...
MSUF_ApplyCombatStateDynamicColor = function()
...
local function MSUF_AnchorCombatCrosshair()
    ...
    --- Wenn Blizzard-Selfhighlight / Nameplates aktiv sind ? an pers?nliche
    --- Nameplate h?ngen und den Offset abh?ngig vom Zoom berechnen.
...
local function EnsureCombatCrosshair()
    ...
    if combatCrosshairFrame.horiz and combatCrosshairFrame.vert then
        ...
        if MSUF_RequestCrosshairRangeRefresh then
            MSUF_RequestCrosshairRangeRefresh()
        elseif MSUF_SetCrosshairRangeTaskEnabled then
...
function MSUF.MSUF_ApplyGameplayFontFromGlobal()
    ApplyFontToCounter()
end
...
local function CreateCombatTimerFrame()
    ...
    if not MSUF._MSUF_CombatTimerModifierFrame then
        local f = CreateFrame("Frame")
        MSUF._MSUF_CombatTimerModifierFrame = f
...
--- 1) Never run background range ticks; range color is event-driven.
...
MSUF_SetCrosshairRangeTaskEnabled = function(enabled)
    ...
    if combatCrosshairFrame.SetScript then
        combatCrosshairFrame:SetScript("OnUpdate", nil)
    end

    if enabled and MSUF_UpdateCombatCrosshairRangeColor then
        MSUF_UpdateCombatCrosshairRangeColor()
...
MSUF_RefreshCrosshairRangeTaskEnabled = function()
    --- No background work: range color updates come from range/target/combat events.
...
function GameplayFeatures.PlayerTotems.Apply(g)
    local fn = MSUF.MSUF_Gameplay_PlayerTotems_Apply
...
function MSUF.MSUF_ApplyGameplayVisuals()
    if MSUF_Gameplay_ApplyNow then
        MSUF_Gameplay_ApplyNow()
    end
end
...
do
    ...
    _specChangeFrame:SetScript("OnEvent", function()
        _UpdateSubRogueCache()
        if MSUF and MSUF.MSUF_RequestGameplayApply then
...
---
--- MSUF: runtime exports
---
do
    MSUF.MSUF_EnsureGameplayDefaults            = EnsureGameplayDefaults
    MSUF.MSUF_GetCombatTimerFrame               = function() return combatFrame end
    MSUF.MSUF_Gameplay_ApplyFontToCounter       = function() ApplyFontToCounter() end
    MSUF.MSUF_Gameplay_ApplyLockState           = function() ApplyLockState() end
    MSUF.MSUF_Gameplay_ApplyCombatTimerAnchorFn = function(g) MSUF_Gameplay_ApplyCombatTimerAnchor(g) end
    MSUF.MSUF_Gameplay_TickCombatTimer          = function() MSUF_Gameplay_TickCombatTimer() end
    MSUF.MSUF_GetCombatTimerAnchorFrame         = function(g) return _MSUF_GetCombatTimerAnchorFrame(g) end
    MSUF.MSUF_SetEnabledMeleeRangeCheck         = function(id) MSUF_SetEnabledMeleeRangeCheck(id) end
    MSUF.MSUF_BuildMeleeSpellCache              = function() MSUF_BuildMeleeSpellCache() end
    MSUF.MSUF_GetMeleeSpellCache                = function() return MSUF_MeleeSpellCache end
    MSUF.MSUF_GetPlayerSpecID                   = MSUF_GetPlayerSpecID
    MSUF.MSUF_GetFirstDanceFrame                = function() return firstDanceFrame end
    MSUF.MSUF_ApplyFirstDanceLockState          = function() _ApplyFirstDanceLockState() end
    MSUF.MSUF_ApplyFirstDanceDisplayMode        = function() _ApplyFirstDanceDisplayMode() end
end
