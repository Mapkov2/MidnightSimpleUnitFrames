...
if self.statusBar and self.statusBar.SetReverseFill then
    ...
    function frame:UpdateAnchor(forceLayout)
        EnsureDBSafe()
        local g = (_G.MSUF_DB and _G.MSUF_DB.general) or {}
        local bossConf = (_G.MSUF_DB and _G.MSUF_DB.boss) or {}
        ...
        if detached then
            -- Detached: anchor to UIParent CENTER, but keep the boss unitframe layout delta.
            local i = tonumber(unit:match("boss(%d+)")) or 1
            local dx, dy = 0, 0
            if type(_G.MSUF_GetBossLayoutDelta) == "function" then
                dx, dy = _G.MSUF_GetBossLayoutDelta(i, bossConf)
                dx, dy = tonumber(dx) or 0, tonumber(dy) or 0
            else
                dy = -((i - 1) * 34)
            end
            layoutDirty = BossSetPointIfChanged(self, "CENTER", UIParent, "CENTER", ox + dx, oy + dy) or layoutDirty
...
function _G.MSUF_ApplyBossCastbarsEnabled()
    ...
    local g = _G.MSUF_DB and _G.MSUF_DB.general
    local shouldUse = _G.MSUF_ShouldUseMSUFCastbar
    local enabled = (type(shouldUse) == "function") and shouldUse("boss", g) or ((not g) or (g.enableBossCastbar ~= false))
