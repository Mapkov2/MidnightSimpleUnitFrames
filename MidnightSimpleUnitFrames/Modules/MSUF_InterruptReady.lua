...
local function _ShowOnUnit(cfg, unit)
    ...
    if cfg ~= _cfgRef then _RefreshCfgCache(cfg) end
    if unit == "target" then return _cfgShowTarget == true and _MSUFOwnsCastbar("target", cfg) end
    if unit == "focus" then return _cfgShowFocus == true and _MSUFOwnsCastbar("focus", cfg) end
    if _cfgShowBoss == true and type(unit) == "string" then
        return (_BOSS_UNITS[unit] == true or unit:sub(1, 4) == "boss") and _MSUFOwnsCastbar("boss", cfg)
    end
    return false
end
...
local function _FillAppliesToUnit(unit, cfg)
    ...
    if unit == "target" or unit == "focus" then return _MSUFOwnsCastbar(unit, cfg) end
    if unit:sub(1, 4) == "boss" then return _MSUFOwnsCastbar("boss", cfg) end
    return false
end
...
local function _TrackFillFrame(frame, state, cfg)
    ...
    if not (_FillColorEnabled(cfg) and active and _FillAppliesToUnit(unit, cfg) and _CastAllowsKickIndicator(frame) and _FillTintRuntimeAvailable()) then
...
RefreshFillCooldownFrames = function(cfg)
    ...
    while frame do
        ...
        if _FillColorEnabled(cfg)
           ...
           and _FillAppliesToUnit(frame.unit, cfg) then
            if frame.UpdateColorForInterruptible then
                frame:UpdateColorForInterruptible()
                didRefresh = true
            end
