...
do
    -- PERF: Pre-allocate closures for EVERY unit token we can ever see.
    -- Previously only covered player/target/focus/boss1-5 → raid1-40 and
    -- party1-4 events allocated a fresh closure per event, ~34k/session.
    -- With full coverage: zero closure allocation at runtime.
    local _knownUnits = {
        "player", "pet", "target", "targettarget", "focus", "focustarget",
        "mouseover",
        "boss1", "boss2", "boss3", "boss4", "boss5",
    }
    for i = 1, 40 do _knownUnits[#_knownUnits + 1] = "raid"  .. i end
    for i = 1, 40 do _knownUnits[#_knownUnits + 1] = "raid"  .. i .. "pet" end
    for i = 1, 4  do _knownUnits[#_knownUnits + 1] = "party" .. i end
    for i = 1, 4  do _knownUnits[#_knownUnits + 1] = "party" .. i .. "pet" end
