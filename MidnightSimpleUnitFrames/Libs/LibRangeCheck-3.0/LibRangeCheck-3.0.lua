...
local function lazyInvalidateRangeCache(maxAge, currentTime)
  if rangeCacheLastSweep + maxAge > currentTime then
    return
  end
  rangeCacheLastSweep = currentTime
  invalidateRangeCache(maxAge)
end
...
local function getCachedRange(unit, noItems, maxCacheAge)
  ...
  local currentTime = GetTime()
  lazyInvalidateRangeCache(5, currentTime)
...
function lib:activate()
  ...
  if self.cacheResetTimer then
    if self.cacheResetTimer.Cancel then
      self.cacheResetTimer:Cancel()
    end
