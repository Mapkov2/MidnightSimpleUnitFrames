...
local function BuildBars(ctx)
        ...
        if type(frames) == "table" then
            for _, frame in pairs(frames) do
                if frame and frame.unit and frame.hpBar then
                    ...
                    if frame.ForceUpdate then frame:ForceUpdate("MSUF2_GRADIENT") end
                    if _G.MSUF_UFCore_UpdatePowerBarFast then _G.MSUF_UFCore_UpdatePowerBarFast(frame) end
                    if MSUF.Bars and MSUF.Bars._ApplyHPGradient then
                        if frame.hpGradients then
                            MSUF.Bars._ApplyHPGradient(frame)
                        elseif frame.hpGradient then
                            MSUF.Bars._ApplyHPGradient(frame.hpGradient)
                        end
                    end
                    if MSUF.Bars and MSUF.Bars.ApplyPowerGradientOnce then
                        frame._msufPowerGradEnabled = nil
                        MSUF.Bars.ApplyPowerGradientOnce(frame)
                    elseif MSUF.Bars and MSUF.Bars._ApplyPowerGradient then
                        if frame.powerGradients then
                            MSUF.Bars._ApplyPowerGradient(frame)
                        elseif frame.powerGradient then
                            MSUF.Bars._ApplyPowerGradient(frame.powerGradient)
                        end
                    end
                end
            end
    ...
    M.BindDropdown(ctx, absorbTex,
        function() return BarScopeGet("absorbBarTexture", ReadG("absorbBarTexture", "")) end,
        function(v)
            BarScopeSet("absorbBarTexture", v or "", "MSUF2_ABSORB_TEXTURE")
    ...
    M.BindDropdown(ctx, healAbsorbTex,
        function() return BarScopeGet("healAbsorbBarTexture", ReadG("healAbsorbBarTexture", "")) end,
        function(v)
            BarScopeSet("healAbsorbBarTexture", v or "", "MSUF2_HEAL_ABSORB_TEXTURE")
