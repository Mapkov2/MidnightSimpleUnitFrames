...
function M.RefreshPinnedPreviews(scroll)
    ...
    for i = 1, #list do
        local record = list[i]
        if record and record.update and (not scroll or record.scroll == scroll) then
            record.update()
        end
    end
end
...
function W.AttachPinnedPreview(body, box, opts)
    ...
    local function ShouldPin()
        ...
        if offset <= (opts.activateAfter or 64) then return false end
        ...
        return bodyTop < (scrollTop + (opts.threshold or 6))
    end

    local function ApplyPinnedState()
        if ShouldPin() then
            local level = ((scroll.GetFrameLevel and scroll:GetFrameLevel()) or 1) + (opts.frameLevelOffset or 80)
            if not pinned then
                pinned = true
                box._msuf2PinnedFloating = true
                box:SetParent(scroll)
                box:ClearAllPoints()
            end
    ...
    local record = { scroll = scroll, update = ApplyPinnedState, restore = Restore, box = box }
