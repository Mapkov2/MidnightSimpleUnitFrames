...
local function EnsureFrameBorderOverlay(mock)
    ...
    local edges = overlay._edges
    for i = 1, #FRAME_BORDER_KEYS do
        local key = FRAME_BORDER_KEYS[i]
...
local function SetFrameBorderShown(mock, shown)
    ...
    local edges = overlay and overlay._edges
    if type(edges) ~= "table" then return end
    for i = 1, #FRAME_BORDER_KEYS do
        Core.SetShownSafe(edges[FRAME_BORDER_KEYS[i]], shown)
    end
end
...
function Core.ApplyFrameBorder(box, border, scale)
    ...
    for i = 1, #FRAME_BORDER_KEYS do
        local edge = edges[FRAME_BORDER_KEYS[i]]
        edge:SetVertexColor(r or 0, g or 0, b or 0, a)
    end
...
local function EnsureBoundsGuideLine(bounds, key)
    if not (bounds and bounds.CreateTexture) then return nil end
    bounds._msufGuideLines = bounds._msufGuideLines or {}
    local line = bounds._msufGuideLines[key]
    if not line then
        line = bounds:CreateTexture(nil, "OVERLAY")
        line:SetTexture(TEX_W8)
        bounds._msufGuideLines[key] = line
    end
    return line
end
...
function Core.ApplyBoundsGuide(box, edgeSize)
    ...
    local top = EnsureBoundsGuideLine(bounds, "top")
    local bottom = EnsureBoundsGuideLine(bounds, "bottom")
    local left = EnsureBoundsGuideLine(bounds, "left")
    local right = EnsureBoundsGuideLine(bounds, "right")
    local r, g, b, a = 1, 0.14, 0.08, 0.95

    if top then
        top:SetVertexColor(r, g, b, a)
        top:ClearAllPoints()
        top:SetPoint("TOPLEFT", bounds, "TOPLEFT", 0, 0)
        top:SetPoint("TOPRIGHT", bounds, "TOPRIGHT", 0, 0)
        top:SetHeight(edgeSize)
    end
    if bottom then
        bottom:SetVertexColor(r, g, b, a)
        bottom:ClearAllPoints()
        bottom:SetPoint("BOTTOMLEFT", bounds, "BOTTOMLEFT", 0, 0)
        bottom:SetPoint("BOTTOMRIGHT", bounds, "BOTTOMRIGHT", 0, 0)
        bottom:SetHeight(edgeSize)
    end
    if left then
        left:SetVertexColor(r, g, b, a)
        left:ClearAllPoints()
        left:SetPoint("TOPLEFT", bounds, "TOPLEFT", 0, 0)
        left:SetPoint("BOTTOMLEFT", bounds, "BOTTOMLEFT", 0, 0)
        left:SetWidth(edgeSize)
    end
    if right then
        right:SetVertexColor(r, g, b, a)
        right:ClearAllPoints()
        right:SetPoint("TOPRIGHT", bounds, "TOPRIGHT", 0, 0)
        right:SetPoint("BOTTOMRIGHT", bounds, "BOTTOMRIGHT", 0, 0)
        right:SetWidth(edgeSize)
    end
    for i = 1, #BOUNDS_GUIDE_KEYS do
        Core.SetShownSafe(bounds._msufGuideLines[BOUNDS_GUIDE_KEYS[i]], true)
    end
end
...
local function PreviewSetRoundedEdgeStackShown(mock, shown)
    ...
    PreviewForEachRoundedEdge(mock, function(edge, i)
        Core.SetShownSafe(edge, i <= count)
    end)
end
...
local function PreviewApplyRoundedEdgeStack(mock, edgeSize)
    ...
    local r, g, b, a = Core.BaseEdgeColor()
    for i = 1, count do
