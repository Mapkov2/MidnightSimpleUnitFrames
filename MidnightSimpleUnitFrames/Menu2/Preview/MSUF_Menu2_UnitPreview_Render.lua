...
function Render.Install(Preview, deps)
    ...
    local PlaceHandleAroundRegions = deps.PlaceHandleAroundRegions or function() return false end
    local PreviewRaidGroupNameAllowed = deps.PreviewRaidGroupNameAllowed or function() return false end
    local PreviewRaidGroupNameText = deps.PreviewRaidGroupNameText or function() return "" end
...
function Preview.Refresh(box, reason)
    ...
    for i = 1, #D.STATUS_PREVIEW do
        ...
        if show then
            ...
            if spec.id == "level" then
                local anchor = (statusCfg and statusCfg.anchor) or ResolveStatusPreviewAnchor(spec, conf, g)
                local x = S(tonumber(statusCfg and statusCfg.x) or tonumber(conf[spec.x]) or tonumber(g[spec.x]) or spec.defaultX or 0)
                local y = S(tonumber(statusCfg and statusCfg.y) or tonumber(conf[spec.y]) or tonumber(g[spec.y]) or spec.defaultY or 0)
                if icon.txt then
