...
local _, MSUF = ...
local M = MSUF.MSUF2
M.InstallClassicAuraPreview = function(dependencies)
local ceil = math.ceil
local floor = math.floor
local max = math.max
local min = math.min
local A3 = dependencies.A3
local AURA_PREVIEW_EDGE_OPTS = dependencies.AURA_PREVIEW_EDGE_OPTS
local AURA_SCOPE_VALID = dependencies.AURA_SCOPE_VALID
local AccessibleNumber = dependencies.AccessibleNumber
local AuraDurationBarColor = dependencies.AuraDurationBarColor
local AurasMenuCombatLocked = dependencies.AurasMenuCombatLocked
local C_Timer = dependencies.C_Timer
local DEBUFF_TYPE_BORDER_PREVIEW_ATLAS = dependencies.DEBUFF_TYPE_BORDER_PREVIEW_ATLAS
local FONT = dependencies.FONT
local GFReadGroup = dependencies.GFReadGroup
local GFReadRoot = dependencies.GFReadRoot
local GroupConf = dependencies.GroupConf
local GroupScopeKinds = dependencies.GroupScopeKinds
local IsGroupScope = dependencies.IsGroupScope
local LaneDefaultMax = dependencies.LaneDefaultMax
local LaneMaxKey = dependencies.LaneMaxKey
local LanePlural = dependencies.LanePlural
local LaneSizeKey = dependencies.LaneSizeKey
local M = dependencies.M
local Model = dependencies.Model
local PreviewHelpers = dependencies.PreviewHelpers
local ReadGroupDebuffTypeBorderMode = dependencies.ReadGroupDebuffTypeBorderMode
local Round = dependencies.Round
local ScopeLabel = dependencies.ScopeLabel
local T = dependencies.T
local TEX_W8 = dependencies.TEX_W8
...
local function BuildMiniAuraPreview(ctx, parent, scope, x, y, width, height, lane, opts)
    ...
    local function RenderPreviewIcon(icon, index, cfg, isBuffIcon, forceText)
        ...
        elseif borderAtlas and icon.dispelBorder.SetAtlas then
            local pad = max(1, floor((cfg.size / 6) + 0.5))
...
return BuildAuraStylePreviewWorkbench, RefreshMiniAuraPreviewNow
end
