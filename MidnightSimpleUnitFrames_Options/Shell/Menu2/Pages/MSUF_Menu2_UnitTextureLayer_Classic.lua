...
local function ApplyHighlightTextureConfig(conf, prefix, enabled)
    ...
    if enabled then
        ...
        for suffix, value in pairs(HIGHLIGHT_TEXTURE_PRESET) do
            ...
            if conf[key] ~= value then conf[key], changed = value, true end
        end
    ...
    elseif HighlightTextureConfigured(conf, prefix) then
        for suffix, value in pairs({ CustomTexturePath = "", CropMode = "FULL", BlendMode = "BLEND" }) do
            local key = prefix .. suffix
            if conf[key] ~= value then conf[key], changed = value, true end
        end
    end
...
local function BuildTextureLayer(ctx, builder, unit)
    ...
    local highlightTexture = W.ToggleAt(generalCard, "Highlight texture", colX, -264, colW - 16)
    M.BindBoolWidget(ctx, highlightTexture,
        ...
        LayerMeta("CustomTexturePath", "highlight_texture"))
    Track(highlightTexture)
    local classColor = W.ToggleAt(generalCard, "Class color", colX, -300, colW - 16)
    M.BindBoolWidget(ctx, classColor,
        function() return GetConf(unit)[Key("ColorMode")] == "CLASS" end,
        function(enabled)
            SetString(unit, Key("ColorMode"), enabled and "CLASS" or "CUSTOM", "MSUF2_TEXLAYER", { preview = true })
            RefreshLayer()
        end,
    ...
    Track(BindLayerDropdown(styleCard, "Texture region", 16, -290, colW - 16,
