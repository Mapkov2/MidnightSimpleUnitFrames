...
local PAGE_HELP = {
    ...
    aura_custom_help = { title = "Custom Auras help", lines = { "Every supported UnitFrame has Custom 1, Custom 2, Custom 3, and its special Player Defensives or Dots on Target container under Auras.", "Setup, lists, layout, deep Style, pandemic warning, and Full-Frame effects remain local to that UnitFrame. Only the corresponding icon theme is global under Appearance > Auras." }, actions = { "Open Target", "Open Player", "Open Focus" } },
    aura_filters_help = { title = "Aura Filter help", lines = { "There is no standalone Aura Filters page anymore. Filters and Black-/Whitelists live directly beside the Buff, Debuff, or Custom container they affect.", "Start with scope and lane: Player/Target/Focus/Boss Buffs or Debuffs, or Party/Raid Buffs or Debuffs. Unit filter toggles are local to the selected frame and lane; group lanes use their own single filter token.", "Hide Permanent removes every no-duration aura in that lane. Exact SpellID blacklists remove one spell where Blizzard permits identity filtering; group category blacklists expand to the same live exact-SpellID exclusions. Timer text, swipe, and duration bars are style only." }, actions = { "Open Target", "Open Group Auras" } },
...
local function AuraScopeClarification()
    return {
        text = "Which Aura frame do you mean?\nCooldown text, stack text, duration bars, ordering, and filters are frame-local. Name Player, Target, Focus, Boss, Party, or Raid (and Buffs or Debuffs when it matters). Global Aura Appearance only controls the shared icon theme.",
        status = "info",
        ...
        nextStep = "For example: open target buff cooldown text; where are raid debuff filters; open global aura icon theme.",
    }
end
...
local function AuraLandingClarification()
    return {
        text = "Which Aura area do you want?\nFor frame-local content and Style, name Player, Target, Focus, Boss, Party, or Raid (and Buffs or Debuffs when it matters). For the shared icon shape, border, and shadow theme, say Global Aura Appearance.",
        status = "info",
        ...
        nextStep = "For example: open target auras; open raid debuff filters; open global aura appearance.",
    }
end
...
local function DirectHelpAnswer(query, opts)
    ...
    then
        return {
            text = "Cooldown display help\nCooldown swipe is the radial overlay that shows time remaining on an icon. Cooldown text is the number shown on top of the icon. MSUF configures Aura cooldown swipe, text size, and offsets on the affected frame's Auras > Style controls.\nExamples: turn on target buff cooldown swipe; set player debuff cooldown text size to 14; move target buff cooldown text up.\nName Player, Target, Focus, Boss, Party, or Raid before opening or changing it. Global Aura Appearance only controls the icon theme.\nYou can ask: Open Target | Open Player | Open Group Auras",
            status = "applied",
