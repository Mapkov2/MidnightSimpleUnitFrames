...
local function ExplicitCustomAuraNavigationPage(text)
    ...
    if scope == "arena" then return "uf_arena", "Arena Auras" end
    ...
    for i = 1, #PAGE_TEXT_TARGETS do
        ...
        if ContainsAny(text, spec.terms) then
            if spec.auraRoute then
                ...
                local units = DetectUnits(text)
                local unit = units[1]
                if unit and (unit == "player" or unit == "target" or unit == "focus" or unit == "boss" or unit == "arena") then
                ...
                then
                    ...
                    if activePage == "uf_player" or activePage == "uf_target"
                        or activePage == "uf_focus" or activePage == "uf_boss" or activePage == "uf_arena"
                    then
                    ...
                    if turn and subjectTurn and turn - subjectTurn >= 0 and turn - subjectTurn <= 3 then
                        ...
                        if frameType == "aura" and (contextUnit == "player" or contextUnit == "target"
                            or contextUnit == "focus" or contextUnit:match("^boss") or contextUnit:match("^arena"))
                        then
                            local contextPage = contextUnit:match("^boss") and "uf_boss"
                                or contextUnit:match("^arena") and "uf_arena"
                                or ("uf_" .. contextUnit)
                        ...
                        if type(setting) == "table" then
                            ...
                            if settingFrameType == "aura" and (settingUnit == "player" or settingUnit == "target"
                                or settingUnit == "focus" or settingUnit:match("^boss") or settingUnit:match("^arena"))
                            then
                                local settingPage = settingUnit:match("^boss") and "uf_boss"
                                    or settingUnit:match("^arena") and "uf_arena"
                                    or ("uf_" .. settingUnit)
                    ...
                    -- Boss/Arena or Party/Raid instead of guessing a global page.
                    return nil, nil
                end
            end
            return spec.page, spec.label
...
local function NonMutatingIntentAnswer(text)
    ...
    if intent == "subjective" then
        ...
        return {
            kind = "answer",
            status = "info",
            text = "Aura filter planning\nI will not guess which buffs or debuffs are useless or important, because that depends on class, content, and preference. Name Player, Target, Focus, Boss, Arena, Party, or Raid plus Buffs or Debuffs, then inspect that frame's Aura Options or name one exact filter. I did not change any aura visibility setting.",
